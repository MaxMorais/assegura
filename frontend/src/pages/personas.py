...
class PersonaPageState:
    ...
    def set_view(self, view: str, persona_data: Optional[Dict[str, Any]] = None) -> None:
...
def _render_personas_list_view(api_client: APIClient, page_state: PersonaPageState):
    ...
    if action == "create_new":
        ...
        st.rerun()
    elif action == "filters_changed" or action == "page_changed" or action == "page_size_changed":
        list_component.load_personas(force_refresh=True)
        st.rerun()
...
def _render_view_persona_view(api_client: APIClient, page_state: PersonaPageState):
    ...
    with col1:
        ...
        st.markdown(f"**Description:** {selected_persona.get('description', 'No description provided')}")
...
def _handle_confirmation_dialogs(api_client: APIClient, page_state: PersonaPageState):
    ...
    if action == "delete_persona":
        ...
        with st.container():
            ...
            st.markdown(f"Are you sure you want to delete persona **{persona.get('name')}**?")
