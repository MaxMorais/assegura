...
def form_field(
    ...
    **kwargs
    ...
    if field_type == "text":
        return st.text_input(label_text, value=value or "", help=help_text, key=key, **kwargs)
    elif field_type == "textarea":
        return st.text_area(label_text, value=value or "", help=help_text, key=key, **kwargs)
    elif field_type == "number":
        return st.number_input(label_text, value=value or 0, help=help_text, key=key, **kwargs)
    elif field_type == "select":
        return st.selectbox(label_text, options=options or [], index=0 if value is None else options.index(value) if value in options else 0, help=help_text, key=key, **kwargs)
    elif field_type == "multiselect":
        return st.multiselect(label_text, options=options or [], default=value or [], help=help_text, key=key, **kwargs)
    elif field_type == "checkbox":
        return st.checkbox(label_text, value=bool(value), help=help_text, key=key, **kwargs)
    elif field_type == "date":
        return st.date_input(label_text, value=value or datetime.now().date(), help=help_text, key=key, **kwargs)
    ...
    for i, action in enumerate(actions):
        with cols[i]:
            ...
            if button_type == 'primary':
                if st.button(action['label'], key=f"{key}_{action['key']}" if key else action['key']):
                    return action['key']
            elif button_type == 'secondary':
                if st.button(action['label'], key=f"{key}_{action['key']}" if key else action['key'], type='secondary'):
                    return action['key']
