        ...
        with col3:
            if st.button("🔄 Toggle Status", key="toggle_status"):
                toggle_activity_status(
                    activity_id, not activity["is_active"], api_client
                )
...
def render_activity_overview(activity: dict[str, Any]):
    ...
    with col2:
        ...
        st.write(
            f"**Complexity:** {complexity_stars} ({activity['complexity_score']}/5)"
        )
        st.write(
            f"**Estimated Duration:** {format_duration(activity['estimated_duration'])}"
        )
    ...
    try:
        ...
        if personas:
            for persona_link in personas:
                ...
                with col1:
                    st.write(
                        f"**{persona_link.get('persona_name', 'Unknown Persona')}**"
                    )
                ...
                with col2:
                    ...
                    priority_emoji = priority_colors.get(
                        persona_link.get("priority", "medium"), "🟡"
                    )
                    st.write(
                        f"{priority_emoji} {persona_link.get('priority', 'medium').title()}"
                    )
                ...
                with col4:
                    if st.button(
                        "🗑️", key=f"unlink_{persona_link['id']}", help="Unlink persona"
                    ):
                        if confirm_action("Unlink this persona from the activity?"):
                            unlink_persona(
                                activity_id, persona_link["persona_id"], api_client
                            )
        ...
        if similar_activities:
            for similar in similar_activities:
                ...
                with col1:
                    ...
                    st.write(
                        f"*{similar['erpnext_module']} - {similar['action_type']}*"
                    )
...
def render_activity_analysis(activity_id: str, api_client: APIClient):
    ...
    if compatibility_activity_id and st.button("🔍 Check Compatibility"):
        try:
            compatibility = api_client.check_activity_compatibility(
                activity_id, compatibility_activity_id
            )
