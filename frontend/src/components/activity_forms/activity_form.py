...
def render_activity_form(api_client: APIClient, activity_id: Optional[str] = None):
    ...
    if is_edit_mode:
        st.subheader(f"✏️ Edit Activity")
    ...
    with st.form("activity_form"):
        ...
        with col2:
            version = render_text_input(
                "Version",
                value=activity_data.get('version', '1.0.0') if activity_data else '1.0.0',
                placeholder="1.0.0",
        ...
        with col2:
            estimated_duration = render_number_input(
                ...
                min_value=1,
                value=activity_data.get('estimated_duration', 60) if activity_data else 60,
        ...
        required_fields_text = st.text_area(
            ...
            value=', '.join(activity_data.get('required_fields', [])) if activity_data else '',
        ...
        success_criteria_text = st.text_area(
            ...
            value=', '.join(activity_data.get('success_criteria', [])) if activity_data else '',
        ...
        with col1:
            prerequisites_text = st.text_area(
                ...
                value=', '.join(activity_data.get('prerequisites', [])) if activity_data else '',
        ...
        with col2:
            postconditions_text = st.text_area(
                ...
                value=', '.join(activity_data.get('postconditions', [])) if activity_data else '',
        ...
        with st.expander("🔬 Advanced Configuration"):
            ...
            validation_rules_text = st.text_area(
                "Validation Rules (JSON)",
                value=json.dumps(activity_data.get('validation_rules', {}), indent=2) if activity_data else '{}',
            ...
            test_data_text = st.text_area(
                "Test Data Requirements (JSON)",
                value=json.dumps(activity_data.get('test_data_requirements', {}), indent=2) if activity_data else '{}',
        ...
        with col3:
            if st.form_submit_button("🔍 Preview/Validate"):
                preview_activity_data(
                    name, description, erpnext_module, action_type, target_doctype,
                    complexity_score, estimated_duration, required_fields_text,
                    success_criteria_text, prerequisites_text, postconditions_text,
                    validation_rules_text, test_data_text, tags_text, is_active
                )
    ...
    if submit_button:
        try:
            ...
            activity_data = prepare_activity_data(
                ...
                complexity_score, estimated_duration, required_fields_text,
                success_criteria_text, prerequisites_text, postconditions_text,
                validation_rules_text, test_data_text, tags_text, is_active, version
            )
            
            if is_edit_mode:
                # Update existing activity
                result = api_client.update_activity(activity_id, activity_data)
                show_success_message(f"Activity '{result['name']}' updated successfully!")
            else:
                # Create new activity
                result = api_client.create_activity(activity_data)
                show_success_message(f"Activity '{result['name']}' created successfully!")
                
                # Clear form by redirecting to activities list
                st.switch_page("pages/activities.py")
        
        except Exception as e:
            show_error_message(f"Error {'updating' if is_edit_mode else 'creating'} activity: {str(e)}")


def prepare_activity_data(
    name: str, description: str, erpnext_module: str, action_type: str,
    target_doctype: str, complexity_score: int, estimated_duration: int,
    required_fields_text: str, success_criteria_text: str, prerequisites_text: str,
    postconditions_text: str, validation_rules_text: str, test_data_text: str,
    tags_text: str, is_active: bool, version: str
) -> Dict[str, Any]:
    """Prepare activity data for API submission."""
    
    # Parse comma-separated lists
    required_fields = [f.strip() for f in required_fields_text.split(',') if f.strip()]
    success_criteria = [c.strip() for c in success_criteria_text.split(',') if c.strip()]
    ...
    # Parse JSON fields
    try:
        validation_rules = json.loads(validation_rules_text) if validation_rules_text.strip() else {}
    except json.JSONDecodeError:
        raise ValueError("Invalid JSON in validation rules")
    
    try:
        test_data_requirements = json.loads(test_data_text) if test_data_text.strip() else {}
    except json.JSONDecodeError:
    ...
    with st.form("quick_activity_form"):
        ...
        with col1:
            ...
            erpnext_module = st.selectbox(
                "Module*",
                ["Accounts", "Stock", "Buying", "Selling", "CRM", "Projects", "Manufacturing", "HR"]
            )
            action_type = st.selectbox(
                "Action*",
                ["create", "read", "update", "delete", "list", "search", "approve", "submit"]
            )
        
        with col2:
            target_doctype = st.text_input("Target DocType*", placeholder="E.g., Customer")
            complexity_score = st.slider("Complexity", 1, 5, 3)
            estimated_duration = st.number_input("Duration (seconds)", min_value=1, value=60)
        ...
        if st.form_submit_button("➕ Create Activity", type="primary"):
            if name and erpnext_module and action_type and target_doctype and description:
                try:
                    activity_data = {
                        'name': name,
                        'description': description,
