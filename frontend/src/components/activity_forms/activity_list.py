...
from ..shared.forms import render_search_form
...
def render_activity_list():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📋 Activities", "📊 Statistics", "🔧 Bulk Actions", "➕ Create New"])
...
def render_activities_tab(api_client: APIClient):
    ...
    try:
        ...
        if activities:
            ...
            start_idx = (st.session_state.activity_page - 1) * st.session_state.activity_per_page + 1
...
def render_activity_filters():
    ...
    selected_module = st.selectbox(
        "ERPNext Module",
        modules,
        index=modules.index(st.session_state.activity_filters.get('erpnext_module', '')),
        key="filter_module"
    )
    if selected_module:
    ...
    action_types = [
        ...
        "export", "import", "approve", "reject", "submit", "cancel", "duplicate",
        "print", "email", "share", "assign", "comment", "attachment", "workflow",
        "permission", "custom"
    ]
    ...
    selected_action = st.selectbox(
        "Action Type",
        action_types,
        index=action_types.index(st.session_state.activity_filters.get('action_type', '')),
        key="filter_action"
    )
    if selected_action:
    ...
    # Complexity filter
    complexity_options = ["", "1 - Very Easy", "2 - Easy", "3 - Medium", "4 - Hard", "5 - Very Hard"]
...
def render_activities_table(activities: List[Dict[str, Any]], api_client: APIClient):
    ...
    if table_data:
        ...
        edited_df = st.data_editor(
            ...
            disabled=["Name", "Module", "Action", "DocType", "Complexity", "Duration", "Status", "Updated", "ID"],
            hide_index=True,
        ...
        if st.session_state.selected_activities:
            ...
            with col1:
                if st.button("👁️ View Details", key="view_selected"):
                    if len(st.session_state.selected_activities) == 1:
                        ...
                        show_info_message("Please select only one activity to view details")
            ...
            with col4:
                if st.button("🗑️ Delete", key="delete_selected", type="secondary"):
                    if confirm_action(f"Delete {len(st.session_state.selected_activities)} activities?"):
...
def render_bulk_actions_tab(api_client: APIClient):
    ...
    with col2:
        ...
        if st.button("🗑️ Delete Activities", key="bulk_delete", type="secondary"):
            ...
            if activity_ids:
                if confirm_action(f"Delete {len(activity_ids)} activities? This action cannot be undone."):
...
def render_activity_pagination(total_count: int):
    ...
    total_pages = (total_count + st.session_state.activity_per_page - 1) // st.session_state.activity_per_page
    ...
    if total_pages > 1:
        ...
        with col4:
            if st.button("⏩ Next", disabled=st.session_state.activity_page == total_pages):
                st.session_state.activity_page += 1
                st.rerun()
        
        with col5:
            if st.button("⏭️ Last", disabled=st.session_state.activity_page == total_pages):
...
def bulk_update_status(activity_ids: List[str], is_active: bool, api_client: APIClient):
    ...
    try:
        ...
        show_success_message(f"Set {result['updated_count']} activities to {status_text}")
