...
class PersonaListComponent:
    ...
    def render_persona_list(self,
                           ...
                           on_persona_delete: Optional[Callable[[Dict[str, Any]], None]] = None) -> Optional[str]:
        ...
        with container:
            ...
            if show_title:
                ...
                with col1:
                    ...
                    if total > 0:
                        st.caption(f"Showing {len(list_data.get('personas', []))} of {total} personas")
                ...
                with col3:
                    if st.button("➕ Create New", type="primary", help="Create new persona"):
            ...
            # Persona list
            action_taken = self._render_persona_table(
    ...
    def _render_bulk_actions(self) -> Optional[str]:
        ...
        with col5:
            if st.button("🗑️ Delete Selected", help="Delete selected personas", type="secondary"):
                return "bulk_delete"
        
        return None
    
    def _render_persona_table(self,
                             on_persona_click: Optional[Callable[[Dict[str, Any]], None]] = None,
                             on_persona_edit: Optional[Callable[[Dict[str, Any]], None]] = None,
                             on_persona_delete: Optional[Callable[[Dict[str, Any]], None]] = None) -> Optional[str]:
        ...
        with col1:
            selection = self.list_state.get_selection()
            select_all = st.checkbox("", value=selection.get("select_all", False), key="select_all_personas")
        ...
        for i, persona in enumerate(personas):
            ...
            with col1:
                ...
                if selected != is_selected:
                    ...
                    self.list_state.set_selection({"selected_personas": new_selected, "select_all": False})
                    st.rerun()
            
            with col2:
                # Persona name (clickable)
                if on_persona_click:
                    if st.button(persona["name"], key=f"persona_name_{i}", help="Click to view details"):
            ...
            with col6:
                ...
                with action_col2:
                    if st.button("🗑️", key=f"delete_persona_{i}", help="Delete persona"):
    ...
    def _render_pagination(self) -> Optional[str]:
        ...
        with col1:
            st.markdown(f"Page {current_page} of {total_pages} • Items {start_item}-{end_item} of {total}")
    ...
    def _get_available_roles(self) -> List[str]:
        ...
        return [
            ...
            "Purchase Manager", "Purchase User", "Stock Manager", "Stock User",
            "Item Manager", "Accounts Manager", "Accounts User", "HR Manager",
            "HR User", "Customer", "Supplier"
        ]
    ...
    def load_personas(self, force_refresh: bool = False) -> bool:
        ...
        try:
            ...
            if not force_refresh and list_data.get("personas") and list_data.get("last_updated"):
                return True
...
def render_persona_list(api_client: APIClient,
                       container: Optional[DeltaGenerator] = None,
                       **kwargs) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
