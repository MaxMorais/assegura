...
class PersonaFormComponent:
    ...
    def _load_erpnext_roles(self) -> List[str]:
        ...
        return [
            # Core System Roles
            "Administrator", "System Manager", "User", "Guest",
            ...
            "Accounts Manager", "Accounts User", "Auditor", "Invoice Manager",
            
            # HR Roles
            "HR Manager", "HR User", "Employee", "Leave Approver",
            ...
            # Support Roles
            "Support Team", "Maintenance Manager", "Maintenance User",
    ...
    def render_creation_form(self, 
                           ...
                           show_title: bool = True) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        with container:
            if show_title:
                ...
                st.markdown("Define a new test persona with specific ERPNext roles and permissions.")
            ...
            with st.form("create_persona_form", clear_on_submit=False):
                ...
                if selected_roles:
                    with st.expander("ℹ️ Selected Roles Information"):
                        for role in selected_roles:
                            st.markdown(f"**{role}**: {self._get_role_description(role)}")
                ...
                with col2:
                    validate_only = st.form_submit_button(
                        "🔍 Validate Only",
                        help="Check persona data without creating"
                    )
    ...
    def _validate_form_data(self, data: Dict[str, Any]) -> bool:
        ...
        if not validate_required_field(data.get("name", ""), "Persona name is required"):
        ...
        if name and not validate_text_length(name, min_length=2, max_length=100):
            self.form_state.add_error("name", "Persona name must be 2-100 characters long")
            is_valid = False
        
        # ERPNext roles validation
        roles = data.get("erpnext_roles", [])
        if not roles:
            self.form_state.add_error("erpnext_roles", "At least one ERPNext role is required")
            is_valid = False
        
        # Description length validation
        description = data.get("description", "")
        if description and len(description) > 1000:
            self.form_state.add_error("description", "Description cannot exceed 1000 characters")
            is_valid = False
        
        # Permissions format validation
        permissions = data.get("permissions", "")
        if permissions and not self._validate_permissions_format(permissions):
            self.form_state.add_error("permissions", "Invalid permissions format. Use comma-separated values.")
            is_valid = False
            ...
            if validation_result.get("is_valid", False):
                ...
                if warnings:
                    ...
                    for warning in warnings:
                        st.markdown(f"- **{warning.get('field', 'General')}**: {warning.get('message', '')}")
                ...
                if suggestions:
                    ...
                    for suggestion in suggestions:
                        st.markdown(f"- **{suggestion.get('field', 'General')}**: {suggestion.get('message', '')}")
            else:
                st.error("❌ Persona data has validation errors:")
                errors = validation_result.get("errors", [])
                for error in errors:
                    st.markdown(f"- **{error.get('field', 'General')}**: {error.get('message', '')}")
    ...
    def _handle_creation_submit(self, validate_first: bool = True) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    def _get_role_description(self, role: str) -> str:
        ...
        return role_descriptions.get(role, "Standard ERPNext role with module-specific permissions")


def render_persona_form(api_client: APIClient, 
                       mode: str = "create",
                       persona_data: Optional[Dict[str, Any]] = None,
                       container: Optional[DeltaGenerator] = None) -> Tuple[bool, Optional[Dict[str, Any]]]:
