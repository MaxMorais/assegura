...
class PersonaCache:
    ...
    def set_list(self, cache_key: str, list_data: dict[str, Any]) -> None:
        """Cache list data."""
        self._list_cache[cache_key] = {"data": list_data, "timestamp": datetime.now()}
    ...
    def get_statistics(self) -> Optional[dict[str, Any]]:
        ...
        if (
            self._stats_cache
            and self._stats_timestamp
            and not self._is_expired(self._stats_timestamp)
        ):
...
class PersonaService:
    ...
    async def create_persona(self, persona_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self.api_client.post(
                endpoint=self.base_url, data=persona_data
            )
    ...
    async def update_persona(
        ...
        try:
            ...
            response = await self.api_client.put(
                endpoint=f"{self.base_url}/{persona_id}", data=update_data
            )
    ...
    async def delete_persona(self, persona_id: str) -> None:
        ...
        try:
            ...
            await self.api_client.delete(endpoint=f"{self.base_url}/{persona_id}")
    ...
    async def get_persona_summary(
        ...
        try:
            ...
            raise PersonaAPIError(
                f"Failed to retrieve persona summary: {str(e)}"
            ) from e
    ...
    async def validate_persona_data(
        ...
        try:
            ...
            response = await self.api_client.post(
                endpoint=f"{self.base_url}/validate", data=persona_data
            )

            logger.info(
                f"Persona validation completed: {response.get('is_valid', False)}"
            )
            return response
    ...
    async def get_persona_suggestions(
        ...
        try:
            ...
            response = await self.api_client.post(
                endpoint=f"{self.base_url}/suggestions", data=request_data
            )

            logger.info(
                f"Retrieved {len(response.get('suggestions', []))} persona suggestions"
            )
            return response
    ...
    async def get_persona_statistics(self, use_cache: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            raise PersonaAPIError(
                f"Failed to retrieve persona statistics: {str(e)}"
            ) from e
    ...
    async def find_similar_personas(
        self, persona_id: str, similarity_threshold: float = 0.7
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            response = await self.api_client.get(
                endpoint=f"{self.base_url}/{persona_id}/similar", params=params
            )
    ...
    def _build_list_cache_key(
        ...
        erpnext_role: Optional[str],
    ) -> str:
        """Build cache key for list requests."""
        key_parts = [f"limit:{limit}", f"offset:{offset}"]
...
def create_persona_service(
    api_client: APIClient, cache_ttl_minutes: int = 30
) -> PersonaService:
