...
class TestActivityIntegration:
    ...
    @pytest.fixture
    def link_repository(self, db_session: Session) -> SQLAlchemyActivityPersonaLinkRepository:
    ...
    async def test_create_activity_flow(
        ...
        assert created_activity.complexity_score == sample_activity_data["complexity_score"]
        ...
        retrieved_activity = await activity_service.get_activity_by_id(created_activity.id)
    ...
    async def test_activity_crud_operations(
        ...
        updated_activity = await activity_service.update_activity(activity_id, update_request)
    ...
    async def test_activity_filtering_and_search(
        ...
        activities_data = [
            {**sample_activity_data, "name": "Sales Invoice Create", "erpnext_module": "Accounts", "action_type": "create", "complexity_score": 3},
            {**sample_activity_data, "name": "Purchase Order Update", "erpnext_module": "Buying", "action_type": "update", "complexity_score": 2},
            {**sample_activity_data, "name": "Customer List View", "erpnext_module": "CRM", "action_type": "list", "complexity_score": 1},
            {**sample_activity_data, "name": "Invoice Delete", "erpnext_module": "Accounts", "action_type": "delete", "complexity_score": 4, "is_active": False}
        ]
        ...
        accounts_result = await activity_service.list_activities(accounts_filter, page=1, per_page=10)
        ...
        create_result = await activity_service.list_activities(create_filter, page=1, per_page=10)
        ...
        low_complexity_result = await activity_service.list_activities(low_complexity_filter, page=1, per_page=10)
        ...
        active_result = await activity_service.list_activities(active_filter, page=1, per_page=10)
        ...
        search_result = await activity_service.list_activities(search_filter, page=1, per_page=10)
        assert search_result.total >= 2  # Should find activities with "invoice" in name
        
        # Test combined filters
        combined_filter = ActivityFilterRequest(erpnext_module="Accounts", is_active=True)
        combined_result = await activity_service.list_activities(combined_filter, page=1, per_page=10)
    ...
    async def test_activity_bulk_operations(
        ...
        updated_count = await activity_service.bulk_update_activity_status(activity_ids[:3], False)
    ...
    async def test_activity_statistics(
        ...
        activities_data = [
            {**sample_activity_data, "name": "Stats Test 1", "erpnext_module": "Accounts", "action_type": "create", "complexity_score": 1, "estimated_duration": 60},
            {**sample_activity_data, "name": "Stats Test 2", "erpnext_module": "Accounts", "action_type": "update", "complexity_score": 2, "estimated_duration": 90},
            {**sample_activity_data, "name": "Stats Test 3", "erpnext_module": "Stock", "action_type": "create", "complexity_score": 3, "estimated_duration": 120, "is_active": False},
            {**sample_activity_data, "name": "Stats Test 4", "erpnext_module": "Stock", "action_type": "delete", "complexity_score": 4, "estimated_duration": 150},
        ]
    ...
    async def test_activity_validation_and_business_rules(
        ...
        invalid_data = {**activity_data, "name": "Invalid Complexity", "complexity_score": 6}
        with pytest.raises(Exception):  # Should raise ValidationError
            invalid_request = ActivityCreateRequest(**invalid_data)
            await activity_service.create_activity(invalid_request)
        
        # Test invalid duration
        invalid_data = {**activity_data, "name": "Invalid Duration", "estimated_duration": 0}
        with pytest.raises(Exception):  # Should raise ValidationError
            invalid_request = ActivityCreateRequest(**invalid_data)
            await activity_service.create_activity(invalid_request)
    ...
    async def test_activity_persona_relationships(
        ...
        from src.application.dto.activity_schemas import ActivityPersonaLinkCreateRequest
        ...
        high_priority_link = next((l for l in activity_personas if l.priority == "high"), None)
        ...
        with pytest.raises(Exception):  # Should raise ActivityPersonaLinkAlreadyExistsError
...
class TestActivityAPIIntegration:
    ...
    def sample_activity_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_criteria": ["Customer created successfully", "Customer ID generated"],
            "complexity_score": 2,
    ...
    async def test_create_activity_api(
        ...
        response = await async_client.post("/api/v1/activities/", json=sample_activity_data)
    ...
    async def test_get_activity_api(
        ...
        create_response = await async_client.post("/api/v1/activities/", json=sample_activity_data)
    ...
    async def test_list_activities_api(
        ...
        # Create multiple activities
        activities_data = [
            {**sample_activity_data, "name": "API List Test 1", "erpnext_module": "CRM"},
            {**sample_activity_data, "name": "API List Test 2", "erpnext_module": "Accounts"},
            {**sample_activity_data, "name": "API List Test 3", "erpnext_module": "CRM", "action_type": "update"}
        ]
        ...
        data = response.json()
        assert data["total"] >= 2
        assert all(activity["erpnext_module"] == "CRM" for activity in data["activities"])
    ...
    async def test_update_activity_api(
        ...
        update_response = await async_client.put(f"/api/v1/activities/{activity_id}", json=update_data)
    ...
    async def test_bulk_operations_api(
        ...
        response = await async_client.post("/api/v1/activities/bulk/update-status", json=bulk_update_data)
        assert response.status_code == 200
        data = response.json()
