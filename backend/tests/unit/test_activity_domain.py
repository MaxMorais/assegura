...
class TestActivityEntity:
    ...
    def test_estimated_duration_validation(self, valid_activity_data: dict[str, Any]):
        ...
        for duration in valid_durations:
            activity = Activity(
                **{**valid_activity_data, "estimated_duration": duration}
            )
    ...
    def test_validation_rules_property(self, valid_activity_data: dict[str, Any]):
        ...
        activity = Activity(
            **{**valid_activity_data, "validation_rules": validation_rules}
        )
    ...
    def test_test_data_requirements_property(self, valid_activity_data: dict[str, Any]):
        ...
        activity = Activity(
            **{**valid_activity_data, "test_data_requirements": test_data}
        )
        assert activity.test_data_requirements == test_data
        assert (
            activity.test_data_requirements_json
            == '{"customer": "test_customer_001", "items": [{"item_code": "ITEM001", "qty": 1}]}'
        )
...
class TestActivityService:
    ...
    def test_validate_activity_configuration(
        self, activity_service: ActivityService, valid_activity_data: dict[str, Any]
    ):
    ...
    def test_analyze_activity_relationships(
        ...
        assert any(
            rel["from_activity_name"] == "Create Customer"
            and rel["to_activity_name"] == "Create Sales Invoice"
    ...
    def test_check_prerequisite_satisfaction(
        ...
        result = activity_service.check_prerequisite_satisfaction(
            activity, satisfied_state
        )
        assert result["satisfied"] is True
        assert len(result.get("missing_prerequisites", [])) == 0

        # Test with unsatisfied prerequisites
        unsatisfied_state = {"customer_exists": False, "items_available": True}

        result = activity_service.check_prerequisite_satisfaction(
            activity, unsatisfied_state
        )
    ...
    def test_estimate_execution_time(
        ...
        high_complexity_estimation = activity_service.estimate_execution_time(
            high_complexity_activity
        )
        assert (
            high_complexity_estimation["adjusted_duration"]
            > estimation["adjusted_duration"]
        )
