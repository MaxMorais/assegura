...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "complexity_level", sa.String(50), nullable=False, server_default="medium"
        ),
        ...
        sa.CheckConstraint("usage_count >= 0", name="ck_journey_positive_usage_count"),
        # Unique constraints
        sa.UniqueConstraint(
    ...
    op.create_table(
        ...
        sa.Column(
            "can_run_parallel", sa.Boolean(), nullable=False, server_default="false"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["action_id"], ["action_library.id"], ondelete="RESTRICT"
        ),
        # Check constraints
        sa.CheckConstraint("step_number > 0", name="ck_journey_step_positive_number"),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Column(
            "journey_id", postgresql.UUID(as_uuid=True), nullable=False, unique=True
        ),
        ...
        sa.Column(
            "can_execute_parallel", sa.Boolean(), nullable=False, server_default="false"
        ),
        ...
        sa.ForeignKeyConstraint(["journey_id"], ["journeys.id"], ondelete="CASCADE"),
        # Check constraints
        sa.CheckConstraint("total_steps > 0", name="ck_execution_plan_positive_steps"),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "plan_version > 0", name="ck_execution_plan_positive_version"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "execution_status", sa.String(50), nullable=False, server_default="running"
        ),
        sa.Column("final_result", sa.String(50), nullable=True),
        sa.Column(
            "total_steps_executed", sa.Integer(), nullable=False, server_default="0"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "journey_execution_id", postgresql.UUID(as_uuid=True), nullable=False
        ),
        ...
        # Foreign key constraints
        sa.ForeignKeyConstraint(
            ["journey_execution_id"], ["journey_executions.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["journey_step_id"], ["journey_steps.id"], ondelete="CASCADE"
        ),
        # Check constraints
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "execution_order > 0", name="ck_step_execution_positive_order"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_journey_persona_activity", "journeys", ["persona_id", "activity_id"]
    )
    op.create_index(
        "ix_journey_execution_status_active",
        "journeys",
    ...
    op.create_index(
        ...
        "journeys",
        ["complexity_level", "estimated_duration_minutes"],
    )
    op.create_index(
        "ix_journey_usage_tracking", "journeys", ["usage_count", "last_used_date"]
    )
    op.create_index(
        "ix_journey_created_updated", "journeys", ["created_at", "updated_at"]
    )

    # JSON indexes for journeys
    op.create_index(
        "ix_journey_metadata_gin", "journeys", ["metadata"], postgresql_using="gin"
    )
    op.create_index(
        "ix_journey_prerequisites_gin",
        "journeys",
        ["prerequisites"],
    ...
    op.create_index(
        "ix_journey_step_execution_status", "journey_steps", ["execution_status"]
    )
    op.create_index(
        "ix_journey_step_can_run_parallel", "journey_steps", ["can_run_parallel"]
    )
    op.create_index("ix_journey_step_is_critical", "journey_steps", ["is_critical"])

    # Composite indexes for journey_steps
    op.create_index(
        "ix_journey_step_journey_order", "journey_steps", ["journey_id", "step_number"]
    )
    op.create_index("ix_journey_step_action_lookup", "journey_steps", ["action_id"])
    op.create_index(
        "ix_journey_step_parallelization",
        "journey_steps",
        ["can_run_parallel", "is_critical"],
    )

    # JSON indexes for journey_steps
    op.create_index(
        "ix_journey_step_parameters_gin",
        "journey_steps",
        ["parameters"],
        postgresql_using="gin",
    )
    ...
    op.create_index(
        "ix_journey_step_depends_on_gin",
        "journey_steps",
        ["depends_on_steps"],
        postgresql_using="gin",
    )
    ...
    op.create_index(
        "ix_execution_plan_journey", "journey_execution_plans", ["journey_id"]
    )
    op.create_index(
        "ix_execution_plan_current", "journey_execution_plans", ["is_current"]
    )
    op.create_index(
        "ix_execution_plan_duration_complexity",
        "journey_execution_plans",
        ["estimated_duration_seconds", "complexity_score"],
    )
    op.create_index(
        "ix_execution_plan_parallelization",
        "journey_execution_plans",
    ...
    op.create_index(
        "ix_journey_execution_journey_id", "journey_executions", ["journey_id"]
    )
    op.create_index(
        "ix_journey_execution_execution_number",
        "journey_executions",
        ["execution_number"],
    )
    op.create_index(
        "ix_journey_execution_execution_batch_id",
        "journey_executions",
        ["execution_batch_id"],
    )
    op.create_index(
        "ix_journey_execution_started_at", "journey_executions", ["started_at"]
    )
    op.create_index(
        "ix_journey_execution_completed_at", "journey_executions", ["completed_at"]
    )
    op.create_index(
        "ix_journey_execution_execution_status",
        "journey_executions",
        ["execution_status"],
    )
    op.create_index(
        "ix_journey_execution_final_result", "journey_executions", ["final_result"]
    )
    op.create_index(
        "ix_journey_execution_execution_environment",
        "journey_executions",
        ["execution_environment"],
    )
    op.create_index(
        "ix_journey_execution_executed_by_user_id",
        "journey_executions",
        ["executed_by_user_id"],
    )
    ...
    op.create_index(
        "ix_journey_execution_journey_number",
        "journey_executions",
        ["journey_id", "execution_number"],
    )
    op.create_index(
        "ix_journey_execution_batch", "journey_executions", ["execution_batch_id"]
    )
    op.create_index(
        "ix_journey_execution_timing",
        "journey_executions",
        ["started_at", "completed_at"],
    )
    op.create_index(
        "ix_journey_execution_status_result",
        "journey_executions",
        ["execution_status", "final_result"],
    )
    op.create_index(
        "ix_journey_execution_performance",
        "journey_executions",
    ...
    op.create_index(
        "ix_step_execution_journey_execution_id",
        "journey_step_executions",
        ["journey_execution_id"],
    )
    op.create_index(
        "ix_step_execution_journey_step_id",
        "journey_step_executions",
        ["journey_step_id"],
    )
    op.create_index(
        "ix_step_execution_step_number", "journey_step_executions", ["step_number"]
    )
    op.create_index(
        "ix_step_execution_execution_order",
        "journey_step_executions",
        ["execution_order"],
    )
    op.create_index(
        "ix_step_execution_started_at", "journey_step_executions", ["started_at"]
    )
    op.create_index(
        "ix_step_execution_completed_at", "journey_step_executions", ["completed_at"]
    )
    op.create_index(
        "ix_step_execution_execution_status",
        "journey_step_executions",
        ["execution_status"],
    )
    op.create_index(
        "ix_step_execution_execution_result",
        "journey_step_executions",
        ["execution_result"],
    )
    op.create_index(
        "ix_step_execution_retry_count", "journey_step_executions", ["retry_count"]
    )

    # Composite indexes for journey_step_executions
    op.create_index(
        "ix_step_execution_journey_execution",
        "journey_step_executions",
        ["journey_execution_id"],
    )
    op.create_index(
        "ix_step_execution_journey_step", "journey_step_executions", ["journey_step_id"]
    )
    op.create_index(
        "ix_step_execution_step_order",
        "journey_step_executions",
        ["step_number", "execution_order"],
    )
    op.create_index(
        "ix_step_execution_timing",
        "journey_step_executions",
        ["started_at", "completed_at"],
    )
    op.create_index(
        "ix_step_execution_status_result",
        "journey_step_executions",
        ["execution_status", "execution_result"],
    )
    op.create_index(
        "ix_step_execution_performance", "journey_step_executions", ["duration_seconds"]
    )
    ...
    op.create_index(
        "ix_step_execution_artifacts_gin",
        "journey_step_executions",
        ["step_artifacts"],
        postgresql_using="gin",
    )
