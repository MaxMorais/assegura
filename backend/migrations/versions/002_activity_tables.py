...
def upgrade() -> None:
    ...
    op.create_index('idx_activities_complexity_score', 'activities', ['complexity_score'])
    ...
    op.create_index('idx_activity_module_type', 'activities', ['erpnext_module', 'action_type'])
    op.create_index('idx_activity_complexity_duration', 'activities', ['complexity_score', 'estimated_duration'])
    op.create_index('idx_activity_active_updated', 'activities', ['is_active', 'updated_at'])
    
    # Create indexes for activity_persona_links table
    op.create_index('idx_activity_persona_links_persona_id', 'activity_persona_links', ['persona_id'])
    op.create_index('idx_activity_persona_links_activity_id', 'activity_persona_links', ['activity_id'])
    op.create_index('idx_activity_persona_links_priority', 'activity_persona_links', ['priority'])
    op.create_index('idx_persona_priority', 'activity_persona_links', ['persona_id', 'priority'])
    op.create_index('idx_activity_order', 'activity_persona_links', ['activity_id', 'execution_order'])
    ...
    op.create_unique_constraint('uq_persona_activity', 'activity_persona_links', ['persona_id', 'activity_id'])
    
    # Create foreign key constraints
    op.create_foreign_key(
        'fk_activity_persona_links_activity_id',
        'activity_persona_links', 'activities',
        ['activity_id'], ['id'],
        ondelete='CASCADE'
    )
