...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "char_length(subdomain) >= 3", name="ck_tenants_subdomain_length"
        ),
        sa.CheckConstraint("char_length(name) >= 1", name="ck_tenants_name_length"),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("email", "tenant_id", name="uq_consultants_email_tenant"),
        # Constraints
        sa.CheckConstraint(
            "role IN ('admin', 'consultant', 'viewer')", name="ck_consultants_role"
        ),
        sa.CheckConstraint(
            "char_length(email) >= 5", name="ck_consultants_email_length"
        ),
        sa.CheckConstraint(
            "char_length(first_name) >= 1", name="ck_consultants_first_name_length"
        ),
        sa.CheckConstraint(
            "char_length(last_name) >= 1", name="ck_consultants_last_name_length"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Index(
            "ix_auth_audit_log_consultant_timestamp", "consultant_id", "timestamp"
        ),
