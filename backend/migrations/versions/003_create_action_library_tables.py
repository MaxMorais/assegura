...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "default_timeout_seconds", sa.Integer(), nullable=False, server_default="30"
        ),
        sa.Column(
            "default_retry_count", sa.Integer(), nullable=False, server_default="0"
        ),
        ...
        sa.CheckConstraint(
            "usage_count >= 0", name="ck_action_non_negative_usage_count"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Column(
            "action_id", postgresql.UUID(as_uuid=True), nullable=False, unique=True
        ),
        ...
        sa.Column(
            "average_duration_seconds", sa.Float(), nullable=False, server_default="0.0"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["action_id"], ["action_library.id"], ondelete="CASCADE"
        ),
        # Check constraints
        sa.CheckConstraint(
            "total_executions >= 0", name="ck_metrics_non_negative_total"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        # Check constraints
        sa.CheckConstraint("version_number > 0", name="ck_version_positive_number"),
        # Unique constraints
        sa.UniqueConstraint(
    ...
    op.create_table(
        ...
        # Foreign key constraints
        sa.ForeignKeyConstraint(
            ["source_action_id"], ["action_library.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["target_action_id"], ["action_library.id"], ondelete="CASCADE"
        ),
        # Check constraints
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_action_library_bdd_step_type", "action_library", ["bdd_step_type"]
    )
    ...
    op.create_index(
        "ix_action_library_erpnext_doctype", "action_library", ["erpnext_doctype"]
    )
    op.create_index("ix_action_library_usage_count", "action_library", ["usage_count"])
    op.create_index(
        "ix_action_library_last_used_date", "action_library", ["last_used_date"]
    )
    op.create_index(
        "ix_action_library_success_rate", "action_library", ["success_rate"]
    )
    ...
    op.create_index(
        "ix_action_classification",
        "action_library",
        ["action_type", "bdd_step_type", "category"],
    )
    op.create_index(
        "ix_action_usage_metrics", "action_library", ["usage_count", "last_used_date"]
    )
    op.create_index(
    ...
    op.create_index(
        ...
        "action_library",
        ["default_timeout_seconds", "default_retry_count"],
    )
    op.create_index(
        "ix_action_status_active", "action_library", ["is_active", "created_at"]
    )

    # JSON indexes for action_library
    op.create_index(
        "ix_action_implementation_gin",
        "action_library",
        ["implementation"],
    ...
    op.create_index(
        "ix_action_prerequisites_gin",
        "action_library",
        ["prerequisites"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_action_postconditions_gin",
        "action_library",
        ["postconditions"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_action_tags_gin", "action_library", ["tags"], postgresql_using="gin"
    )
    op.create_index(
        "ix_action_metadata_gin", "action_library", ["metadata"], postgresql_using="gin"
    )
    op.create_index(
        "ix_action_ui_selectors_gin",
        "action_library",
        ["ui_selectors"],
        postgresql_using="gin",
    )
    ...
    op.create_index(
        "ix_metrics_last_execution_date",
        "action_execution_metrics",
        ["last_execution_date"],
    )
    op.create_index(
        "ix_metrics_last_success_date",
        "action_execution_metrics",
        ["last_success_date"],
    )
    op.create_index(
        "ix_metrics_last_failure_date",
        "action_execution_metrics",
        ["last_failure_date"],
    )
    op.create_index(
        "ix_metrics_stability_rating", "action_execution_metrics", ["stability_rating"]
    )
    ...
    op.create_index(
        "ix_metrics_timing",
        "action_execution_metrics",
        ["average_duration_seconds", "last_execution_date"],
    )
    op.create_index(
        "ix_metrics_reliability",
        "action_execution_metrics",
        ["reliability_score", "stability_rating"],
    )
    ...
    op.create_index(
        "ix_version_implementation_hash", "action_versions", ["implementation_hash"]
    )
    op.create_index(
        "ix_version_parameters_schema_hash",
        "action_versions",
    ...
    op.create_index(
        "ix_version_backward_compatible", "action_versions", ["backward_compatible"]
    )

    # Composite indexes for action_versions
    op.create_index(
        "ix_version_action_number", "action_versions", ["action_id", "version_number"]
    )
    op.create_index(
        "ix_version_hashes",
        "action_versions",
        ["implementation_hash", "parameters_schema_hash"],
    )
    op.create_index("ix_version_status", "action_versions", ["is_current", "is_stable"])

    # JSON indexes for action_versions
    op.create_index(
        "ix_version_change_details_gin",
        "action_versions",
        ["change_details"],
        postgresql_using="gin",
    )
    ...
    op.create_index(
        "ix_relationship_source_action_id", "action_relationships", ["source_action_id"]
    )
    op.create_index(
        "ix_relationship_target_action_id", "action_relationships", ["target_action_id"]
    )
    op.create_index(
        "ix_relationship_relationship_type",
        "action_relationships",
        ["relationship_type"],
    )
    op.create_index(
        "ix_relationship_relationship_strength",
        "action_relationships",
        ["relationship_strength"],
    )
    ...
    op.create_index(
        "ix_relationship_conditions_gin",
        "action_relationships",
        ["conditions"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_relationship_metadata_gin",
        "action_relationships",
        ["metadata"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_relationship_context_tags_gin",
        "action_relationships",
        ["context_tags"],
        postgresql_using="gin",
    )
    ...
    op.create_index(
        "ix_usage_usage_context", "action_usage_tracking", ["usage_context"]
    )
    op.create_index(
        "ix_usage_usage_environment", "action_usage_tracking", ["usage_environment"]
    )
    op.create_index("ix_usage_used_at", "action_usage_tracking", ["used_at"])
    op.create_index(
        "ix_usage_execution_result", "action_usage_tracking", ["execution_result"]
    )
    ...
    op.create_index(
        "ix_usage_action_time", "action_usage_tracking", ["action_id", "used_at"]
    )
    op.create_index(
        "ix_usage_journey_time", "action_usage_tracking", ["journey_id", "used_at"]
    )
    op.create_index(
        "ix_usage_context_environment",
        "action_usage_tracking",
        ["usage_context", "usage_environment"],
    )
    op.create_index(
        "ix_usage_result_duration",
        "action_usage_tracking",
        ["execution_result", "execution_duration_seconds"],
    )
    op.create_index(
        "ix_usage_session", "action_usage_tracking", ["session_id", "used_at"]
    )
    op.create_index(
        "ix_usage_time_partitioning", "action_usage_tracking", ["used_at"]
    )  # For time-based partitioning

    # JSON indexes for action_usage_tracking
    op.create_index(
        "ix_usage_parameters_gin",
        "action_usage_tracking",
        ["parameters_used"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_usage_metadata_gin",
        "action_usage_tracking",
        ["metadata"],
        postgresql_using="gin",
    )
