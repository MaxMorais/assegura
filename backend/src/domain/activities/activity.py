...
class Activity:
    ...
    def _validate(self) -> None:
        ...
        if not self.name or not self.name.strip():
            errors.append(
                ActivityValidationError("name", "Name is required and cannot be empty")
            )
        ...
        if self.action_type not in valid_action_types:
            errors.append(
                ActivityInvalidActionTypeError(self.action_type, valid_action_types)
            )
        ...
        if (
            ...
            or self.complexity_score < 1
            or self.complexity_score > 5
        ):
            errors.append(
                ActivityValidationError(
                    "complexity_score",
                    "Complexity score must be an integer between 1 and 5",
                )
            )
        ...
        if not isinstance(self.estimated_duration, int) or self.estimated_duration <= 0:
            errors.append(
                ActivityValidationError(
                    "estimated_duration",
                    "Estimated duration must be a positive integer (seconds)",
                )
            )
        ...
        if self.required_fields and not isinstance(self.required_fields, list):
            errors.append(
                ActivityValidationError(
                    "required_fields", "Required fields must be a list"
                )
            )
        ...
        if self.validation_rules and not isinstance(self.validation_rules, dict):
            errors.append(
                ActivityValidationError(
                    "validation_rules", "Validation rules must be a dictionary"
                )
            )
        ...
        if self.success_criteria and not isinstance(self.success_criteria, list):
            errors.append(
                ActivityValidationError(
                    "success_criteria", "Success criteria must be a list"
                )
            )
    ...
    def calculate_similarity(self, other: "Activity") -> float:
        ...
        if self.required_fields and other.required_fields:
            ...
            fields_similarity = (
                len(common_fields) / len(total_fields) if total_fields else 0
            )
        else:
            fields_similarity = (
                1.0 if not self.required_fields and not other.required_fields else 0.0
            )
