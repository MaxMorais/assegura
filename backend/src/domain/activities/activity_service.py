...
from .activity import Activity, ActivityActionType, ActivityComplexity, ActivityPriority
from .erpnext_modules import ERPNextModuleValidator, ERPNextDocTypeHelper
from .exceptions import (
    ActivityValidationError,
    ActivityMultipleValidationError,
    ActivityCompatibilityError,
    ActivityPrerequisiteError,
    ActivityPostconditionError,
...
class ActivityService:
    ...
    def validate_activity_creation(self, activity: Activity) -> None:
        ...
        compatibility = self.module_validator.validate_activity_module_compatibility(
            activity.erpnext_module,
            activity.target_doctype,
        ...
        if abs(activity.complexity_score - expected_complexity) > 2:
            errors.append(ActivityValidationError(
                "complexity_score",
        ...
        if activity.estimated_duration < expected_duration * 0.5 or activity.estimated_duration > expected_duration * 3:
            errors.append(ActivityValidationError(
                "estimated_duration",
    ...
    def calculate_activity_compatibility(self, activity1: Activity, activity2: Activity) -> ActivityCompatibilityResult:
        ...
        if activity1.erpnext_module == activity2.erpnext_module:
            ...
            suggestions.append("Consider grouping activities by module for better workflow coherence")
        ...
        compatibility_factors.append((action_compatibility, 0.2, "Action type compatibility"))
        ...
        if action_compatibility > 0.7:
            ...
            suggestions.append("Consider sequencing activities to create logical workflow")
        ...
        compatibility_factors.append((complexity_compatibility, 0.1, "Similar complexity"))
        
        # Duration compatibility
        duration_ratio = min(activity1.estimated_duration, activity2.estimated_duration) / \
        ...
        confidence = min(1.0, len([f for f in compatibility_factors if f[0] > 0.5]) / len(compatibility_factors))
        ...
        if total_score > 0.8:
            ...
            suggestions.append("These activities can be effectively combined with proper sequencing")
        elif total_score < 0.3:
            suggestions.append("Consider reviewing if these activities belong in the same test scenario")
        ...
        for activity in all_activities:
            ...
            if similarity_score >= similarity_threshold:
                similar_activities.append({
                    ...
                    'similarity_factors': self._analyze_similarity_factors(target_activity, activity)
                })
    ...
    def validate_activity_execution_context(
        ...
        if missing_prerequisites:
            validation_errors.append(f"Missing prerequisites: {', '.join(missing_prerequisites)}")
        
        # Validate required test data availability
        for field in activity.required_fields:
            if field not in context.available_test_data and field not in context.execution_data:
                validation_errors.append(f"Required test data for field '{field}' is not available")
        ...
        if context.erpnext_context:
            ...
            if activity.erpnext_module not in module_compatible:
                validation_errors.append(f"ERPNext module '{activity.erpnext_module}' is not active in context")
    ...
    def generate_activity_suggestions(
        ...
        for module in erpnext_modules:
            module_suggestions = self.module_validator.suggest_modules_for_activity(context, "")
                    ...
                    for doctype in doctypes[:3]:  # Top 3 doctypes per module
                        for action in ['create', 'read', 'update']:
                            activity_suggestion = {
                                ...
                                'confidence': suggestion['confidence'] * 0.8,  # Slightly reduce confidence
                                'required_fields': self.doctype_helper.get_required_fields(doctype),
                                'complexity_score': self._get_expected_complexity_for_action(action, doctype),
                                'estimated_duration': self._get_expected_duration_for_action(action, doctype)
                            }
    ...
    def optimize_activity_sequence(self, activities: List[Activity]) -> List[Activity]:
        ...
        while remaining_activities:
            ...
            ready_activities.sort(key=lambda a: (a.complexity_score, a.estimated_duration))
    ...
    def calculate_activity_statistics(self, activities: List[Activity]) -> Dict[str, Any]:
        ...
        stats['avg_complexity'] = sum(a.complexity_score for a in activities) / len(activities)
        
        # Find most common values
        if stats['by_module']:
            stats['most_common_module'] = max(stats['by_module'], key=stats['by_module'].get)
        if stats['by_action_type']:
            stats['most_common_action'] = max(stats['by_action_type'], key=stats['by_action_type'].get)
        
        return stats
    
    # Private helper methods
    
    def _get_expected_complexity_for_action(self, action_type: str, doctype: str) -> int:
        ...
        complex_doctypes = ['Sales Order', 'Purchase Order', 'Work Order', 'Journal Entry']
    ...
    def _get_expected_duration_for_activity(self, activity: Activity) -> int:
        ...
        base_duration = self._get_expected_duration_for_action(activity.action_type, activity.target_doctype)
        ...
        return int(base_duration * complexity_multiplier.get(activity.complexity_score, 1.0))
    ...
    def _calculate_action_type_compatibility(self, action1: str, action2: str) -> float:
        ...
        if (action1 in read_actions and action2 in read_actions) or \
           (action1 in write_actions and action2 in write_actions) or \
           (action1 in workflow_actions and action2 in workflow_actions):
            return 0.7
    ...
    def _calculate_prerequisite_overlap(self, activity1: Activity, activity2: Activity) -> float:
    ...
    def _analyze_similarity_factors(self, activity1: Activity, activity2: Activity) -> Dict[str, float]:
        """Analyze what makes two activities similar."""
        factors = {}
        
        factors['module_match'] = 1.0 if activity1.erpnext_module == activity2.erpnext_module else 0.0
        factors['action_match'] = 1.0 if activity1.action_type == activity2.action_type else 0.0
        factors['doctype_match'] = 1.0 if activity1.target_doctype == activity2.target_doctype else 0.0
    ...
    def _could_be_prerequisite(self, activity1: Activity, activity2: Activity) -> bool:
        ...
        if activity1.action_type == 'create' and activity2.action_type in ['update', 'delete', 'cancel']:
            return activity1.target_doctype == activity2.target_doctype
        
        # Read/search might be prerequisite for create/update
        if activity1.action_type in ['read', 'search'] and activity2.action_type in ['create', 'update']:
            # Check if they're related doctypes
            return self._calculate_doctype_relationship(activity1.target_doctype, activity2.target_doctype) > 0.5
    ...
    def _could_be_alternative(self, activity1: Activity, activity2: Activity) -> bool:
        ...
        if (activity1.action_type == activity2.action_type and 
            activity1.erpnext_module == activity2.erpnext_module and
            activity1.target_doctype != activity2.target_doctype):
            return True
    ...
    def _calculate_prerequisite_strength(self, prerequisite: Activity, dependent: Activity) -> float:
        ...
        if (prerequisite.action_type == 'create' and 
            dependent.action_type in ['update', 'delete', 'submit', 'cancel']):
            strength += 0.2
        
        return min(1.0, strength)
    
    def _build_dependency_graph(self, activities: List[Activity]) -> Dict[uuid.UUID, Set[uuid.UUID]]:
