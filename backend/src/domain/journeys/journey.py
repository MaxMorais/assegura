...
class Journey(BaseEntity):
    ...
    def move_step(self, from_position: int, to_position: int) -> None:
        ...
        if from_position < 1 or from_position > len(self._steps):
            raise JourneyValidationError(
                f"Invalid from position {from_position}"
            )
        if to_position < 1 or to_position > len(self._steps):
            raise JourneyValidationError(
                f"Invalid to position {to_position}"
            )
    ...
    def _validate(self) -> None:
        ...
        if len(self._description.strip()) < 10:
            raise JourneyValidationError("Journey description must be at least 10 characters")
        
        if len(self._description) > 1000:
            raise JourneyValidationError("Journey description must not exceed 1000 characters")
        ...
        if self._estimated_duration_minutes is not None:
            if self._estimated_duration_minutes < 1:
                raise JourneyValidationError("Estimated duration must be at least 1 minute")
            if self._estimated_duration_minutes > 1440:  # 24 hours
                raise JourneyValidationError("Estimated duration must not exceed 1440 minutes (24 hours)")
    ...
    def _validate_step_sequence(self) -> None:
        ...
        if errors:
            raise JourneyValidationError(f"Step sequence validation failed: {'; '.join(errors)}")
