...
from ..actions.action_types import ActionClassificationService, ActionTypeFactory
from ..actions.parameter_structures import ParameterSetValidator, OutputSetValidator
...
class ActionStepEnhanced:
    ...
    def validate_parameters(self) -> List[str]:
        ...
        validator = ParameterSetValidator([
            param for param in self.action.parameters
        ])
    ...
    def get_effective_retry_count(self) -> int:
        ...
        return self.retry_override if self.retry_override is not None else self.action.retry_count or 0
...
class EnhancedJourney(BaseJourney):
        ...
        **step_kwargs
        ...
        if action.parameters:
            ...
            if param_errors:
                ...
                raise JourneyValidationError(f"Parameter validation failed: {error_msg}")
    ...
    def validate_enhanced_journey(self) -> List[str]:
        ...
        if self._enhanced_steps:
            ...
            sequence_errors = ActionClassificationService.validate_action_sequence(actions)
    ...
    def generate_execution_plan(self) -> JourneyExecutionPlan:
        ...
        total_duration = sum(
            step.get_effective_timeout() + (step.get_effective_retry_count() * step.get_effective_timeout())
            for step in self._enhanced_steps
        )
        
        # Calculate complexity score
        complexity_score = sum(
            ActionClassificationService.get_action_complexity_score(step.action)["total_score"]
            for step in self._enhanced_steps
        )
        ...
        critical_steps = [
            step.step_number for step in self._enhanced_steps 
    ...
    def get_step_by_action_type(self, action_type: ActionType) -> List[ActionStepEnhanced]:
        """Get all steps of specified action type."""
        return [
            step for step in self._enhanced_steps 
    ...
    def get_actions_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_complete_bdd_flow": (
                ActionType.GIVEN.value in action_types and
                ActionType.WHEN.value in action_types and
                ActionType.THEN.value in action_types
            )
        }
