...
from .enhanced_journey import EnhancedJourney, ActionStepEnhanced
...
from ..actions.action_library import Action, ActionType, ImplementationType
from ..actions.action_types import ActionTypeFactory, ActionClassificationService
from ..actions.parameter_structures import ParameterSetValidator, ActionParameterAdvanced
...
class JourneyActionService:
    ...
    async def replace_action_in_step(
        ...
        if new_action.parameters:
            ...
            if param_errors:
                ...
                raise JourneyValidationError(f"Invalid parameters for new action: {error_details}")
    ...
    async def suggest_actions_for_journey(
        ...
        for action_type in missing_types:
            actions = await self.action_repository.search_actions(action_type=action_type)
            for action in actions:
                confidence = await self._calculate_action_fit_score(action, journey, context)
        ...
        for module in current_modules:
            ...
            for action in actions:
                ...
                if any(step.action.action_id == action.action_id for step in journey.enhanced_steps):
                    continue
    ...
    async def optimize_journey_steps(self, journey: EnhancedJourney) -> List[str]:
        ...
        for from_pos, to_pos, reason in reordering_suggestions:
            ...
            optimizations.append(f"Moved step {from_pos} to position {to_pos}: {reason}")
    ...
    async def validate_journey_with_actions(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        for step in journey.enhanced_steps:
            ...
            if hasattr(action, 'is_deprecated') and action.is_deprecated:
                results.append(ValidationResult(
                    ...
                    severity=self.validator.rules[0].severity,  # Use first rule's default severity
    ...
    async def get_journey_execution_plan(self, journey: EnhancedJourney) -> Dict[str, Any]:
        ...
        action_details = []
        for step in journey.enhanced_steps:
            usage_info = await self.action_repository.get_action_usage_stats(step.action.action_id)
            ...
            action_details.append({
                ...
                "complexity_score": ActionClassificationService.get_action_complexity_score(step.action),
    ...
    async def _adapt_parameters(
        ...
        for old_name in remaining_old:
            ...
            for new_name in remaining_new:
                if self._are_parameters_compatible(old_name, new_name, semantic_mappings):
    ...
    async def _calculate_action_fit_score(
        ...
        # Usage statistics
        usage_info = await self.action_repository.get_action_usage_stats(action.action_id)
    ...
    async def _find_redundant_steps(self, journey: EnhancedJourney) -> List[Tuple[ActionStepEnhanced, ActionStepEnhanced]]:
    ...
    async def _suggest_step_reordering(self, journey: EnhancedJourney) -> List[Tuple[int, int, str]]:
    ...
    async def _find_mergeable_steps(self, journey: EnhancedJourney) -> List[Tuple[List[int], Action]]:
    ...
    async def _update_action_usage(self, action_id: UUID, journey_id: UUID, parameters: Dict[str, Any]) -> None:
