...
class JourneyService:
    ...
    def validate_parameter_consistency(self, journey: Journey) -> list[str]:
        ...
        available_outputs: dict[
            str, set[int]
        ] = {}  # parameter_name -> set of step_numbers providing it
        required_inputs: dict[
            str, set[int]
        ] = {}  # parameter_name -> set of step_numbers requiring it
            ...
            for requiring_step in requiring_steps:
                ...
                if not available_before:
                    ...
                    if param_name not in [
                        p.lower().replace(" ", "_") for p in journey.prerequisites
                    ]:
                        errors.append(
            ...
            for param_name in step.parameters.keys():
                for other_step in journey.steps:
                    if (
                        ...
                        and param_name in other_step.expected_outputs
                    ):
    ...
    def validate_business_rules(self, journey: Journey) -> list[str]:
        ...
        for prereq in journey.prerequisites:
            if len(prereq.strip()) < 5:
                errors.append(
                    f"Prerequisite '{prereq}' is too vague. Be more specific."
                )
        ...
        for outcome in journey.expected_outcomes:
            if len(outcome.strip()) < 10:
                errors.append(
                    f"Expected outcome '{outcome}' should be more detailed and measurable."
                )
        ...
        if journey.is_active:
            ...
            if completeness_errors:
                errors.append(
                    "Active journey must be complete (have Given/When/Then steps)"
                )

        return errors

    def get_journey_health_score(
    ...
    def suggest_improvements(self, journey: Journey) -> list[str]:
        ...
        if completeness_errors:
            suggestions.append(
                "Add missing Given/When/Then steps to create a complete BDD scenario"
            )

        if sequence_errors:
            suggestions.append(
                "Review step sequence to ensure logical Given->When->Then flow"
            )

        if parameter_errors:
            suggestions.append(
                "Review parameter dependencies between steps and add missing prerequisites"
            )

        if len(complexity_warnings) > 2:
            suggestions.append(
                "Consider breaking this journey into smaller, focused scenarios"
            )
        ...
        if journey.step_count > 0:
            if not journey.prerequisites:
                suggestions.append(
                    "Add prerequisites to clarify initial conditions needed"
                )

            if not journey.expected_outcomes:
                suggestions.append(
                    "Define expected outcomes to clarify what success looks like"
                )
