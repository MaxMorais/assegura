...
from .enhanced_journey import EnhancedJourney, ActionStepEnhanced, JourneyExecutionStatus
...
from ..actions.action_types import (
    GivenActionPattern, WhenActionPattern, ThenActionPattern,
    ...
    def validate(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        if given_steps and when_steps:
            ...
            if last_given > first_when:
                given_nums = [step.step_number for step in given_steps if step.step_number > first_when]
                results.append(self._create_result(
        ...
        if when_steps and then_steps:
            ...
            if last_when > first_then:
                when_nums = [step.step_number for step in when_steps if step.step_number > first_then]
                results.append(self._create_result(
...
class ERPNextBusinessRule(JourneyValidationRule):
    ...
    def validate(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        for doctype, operations in document_operations.items():
            ...
            if "submit" in operations and "save" not in operations and "create" not in operations:
                results.append(self._create_result(
        ...
        has_login = any(
            "login" in step.action.name.lower() or 
            "authentication" in step.action.description.lower()
    ...
    def _analyze_document_operations(self, journey: EnhancedJourney) -> Dict[str, Set[str]]:
...
class StepComplexityRule(JourneyValidationRule):
    ...
    def validate(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        complex_steps = []
        for step in journey.enhanced_steps:
            complexity = ActionClassificationService.get_action_complexity_score(step.action)
                ...
                for consuming_step in consuming_steps:
                    if consuming_step <= producing_step:
                        results.append(self._create_result(
                            ...
                            suggested_fix=f"Reorder steps or use different variable names"
                        ))
...
class ResourceConflictRule(JourneyValidationRule):
    ...
    def validate(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        parallel_steps = [
            step for step in journey.enhanced_steps 
...
class ActionPatternRule(JourneyValidationRule):
    ...
    def validate(self, journey: EnhancedJourney) -> List[ValidationResult]:
        ...
        for step in given_steps:
            ...
            if not any(keyword in action_name for keyword in ["given", "setup", "prepare", "login", "navigate"]):
                results.append(ValidationResult(
                    rule_name=self.name,
                    severity=ValidationSeverity.INFO,
        ...
        for step in when_steps:
            action_name = step.action.name.lower()
            if not any(keyword in action_name for keyword in ["when", "create", "update", "submit", "process"]):
                results.append(ValidationResult(
                    rule_name=self.name,
                    severity=ValidationSeverity.INFO,
        ...
        for step in then_steps:
            action_name = step.action.name.lower()
            if not any(keyword in action_name for keyword in ["then", "verify", "check", "should", "assert"]):
                results.append(ValidationResult(
                    rule_name=self.name,
                    severity=ValidationSeverity.INFO,
...
class JourneyValidator:
    ...
    def can_execute(self, journey: EnhancedJourney) -> bool:
        ...
        results = self.validate_journey(journey, include_warnings=False, include_info=False)
    ...
    def get_validation_summary(self, journey: EnhancedJourney) -> Dict[str, Any]:
        ...
        results = self.validate_journey(journey, include_warnings=True, include_info=True)
