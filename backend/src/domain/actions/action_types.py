...
class ActionClassificationService:
    ...
    def _validate_given_action(action: Action) -> list[str]:
        ...
        if action.implementation_type == ImplementationType.VERIFICATION:
            errors.append(
                "Given actions should not be verification actions - use Then instead"
            )

        # Given actions should not have complex outputs (they set up state)
        if len(action.expected_outputs) > 3:
            errors.append(
                "Given actions should have minimal outputs (max 3) - they set up state"
            )
        ...
        if not any(keyword in action.name.lower() for keyword in given_keywords):
            errors.append(
                "Given action name should indicate setup/precondition (e.g., 'Given user is logged in')"
            )
    ...
    def _validate_when_action(action: Action) -> list[str]:
        ...
        if (
            len(action.expected_outputs) == 0
            and action.implementation_type != ImplementationType.CLEANUP
        ):
        ...
        if not any(keyword in action.name.lower() for keyword in when_keywords):
            errors.append(
                "When action name should indicate the main action (e.g., 'When user creates invoice')"
            )
    ...
    def _validate_then_action(action: Action) -> list[str]:
        ...
        if action.implementation_type not in [
            ImplementationType.VERIFICATION,
        ...
        if len(action.parameters) > 5:
            errors.append(
                "Then actions should have minimal parameters (max 5) - they verify state"
            )

        # Then actions should not have many outputs (they verify, not create)
        if len(action.expected_outputs) > 2:
            errors.append(
                "Then actions should have minimal outputs (max 2) - they verify results"
            )
        ...
        if not any(keyword in action.name.lower() for keyword in then_keywords):
            errors.append(
                "Then action name should indicate verification (e.g., 'Then invoice should be created')"
            )
    ...
    def suggest_action_type(action: Action) -> ActionType:
        ...
        given_score = sum(
            1 for word in given_indicators if word in name_lower or word in desc_lower
        )
        when_score = sum(
            1 for word in when_indicators if word in name_lower or word in desc_lower
        )
        then_score = sum(
            1 for word in then_indicators if word in name_lower or word in desc_lower
        )
        ...
        if action.implementation_type == ImplementationType.VERIFICATION:
            ...
            given_score += 2
        elif action.implementation_type in [
            ImplementationType.UI_INTERACTION,
            ImplementationType.API_CALL,
        ]:
            when_score += 1
    ...
    @staticmethod
    def get_recommended_parameters_for_pattern(
        pattern: str, action_type: ActionType
        ...
        if action_type == ActionType.GIVEN:
            if pattern == GivenActionPattern.USER_LOGIN.value:
                parameters = [
                    ...
                    ActionParameter(
                        ...
                        "string",
                        required=False,
                        description="Company to login to",
                    ),
                ]
                ...
                parameters = [
                    ...
                    ActionParameter(
                        "data", "object", required=True, description="Document data"
                    ),
                    ActionParameter(
                        "save",
                        "boolean",
                        required=False,
                        description="Save after creation",
                        default_value=True,
                    ),
                ]
                ...
                parameters = [
                    ...
                    ActionParameter(
                        ...
                        "string",
                        required=False,
                        description="Document type to navigate to",
                    ),
                    ActionParameter(
                        "view",
                        "string",
                        required=False,
                        description="View type (list/form/report)",
                    ),
                ]
            ...
            if pattern == WhenActionPattern.CREATE_DOCUMENT.value:
                parameters = [
                    ActionParameter(
                        "doctype", "string", required=True, description="Document type"
                    ),
                    ActionParameter(
                        "data", "object", required=True, description="Document data"
                    ),
                ...
                parameters = [
                    ...
                    ActionParameter(
                        "name", "string", required=True, description="Document name/ID"
                    ),
                    ActionParameter(
                        "data", "object", required=True, description="Updated data"
                    ),
                    ActionParameter(
                        "save",
                        "boolean",
            ...
            if pattern == ThenActionPattern.DOCUMENT_EXISTS.value:
                ...
                parameters = [
                    ...
                    ActionParameter(
                        "field", "string", required=True, description="Field name"
                    ),
                    ActionParameter(
                        "expected_value",
                        "string",
    ...
    @staticmethod
    def get_recommended_outputs_for_pattern(
        pattern: str, action_type: ActionType
    ) -> list[ActionOutput]:
        """
        ...
        if action_type == ActionType.GIVEN:
            if pattern == GivenActionPattern.USER_LOGIN.value:
                ...
                outputs = [
                    ActionOutput(
                        "document_name", "string", "Name/ID of created document"
                    ),
            ...
            if pattern == ThenActionPattern.DOCUMENT_EXISTS.value:
                outputs = [
                    ...
                    ActionOutput(
                        "verification_result", "string", "Verification outcome"
                    ),
                ]
            elif pattern == ThenActionPattern.FIELD_VALUE.value:
                outputs = [
                    ActionOutput(
                        "matches", "boolean", "Whether field value matches expected"
                    ),
        ...
        for i in range(1, len(action_types)):
            if action_types[i] == action_types[i - 1]:
                ...
                if consecutive_same_type > 3:
                    errors.append(
                        f"Too many consecutive {action_types[i].value} actions - consider grouping or refactoring"
                    )
            else:
                consecutive_same_type = 1
    ...
    def get_action_complexity_score(action: Action) -> dict[str, Any]:
        ...
        keyword_score = min(
            len(action.robot_keywords) // 2, 3
        )  # 2 keywords = 1 point, cap at 3

        # Total complexity
        total_score = (
            base_score + impl_score + param_score + output_score + keyword_score
        )
    ...
    def create_given_action(
        ...
        **kwargs,
        ...
        if pattern:
            defaults[
                "parameters"
            ] = ActionClassificationService.get_recommended_parameters_for_pattern(
                pattern.value, ActionType.GIVEN
            )
            defaults[
                "expected_outputs"
            ] = ActionClassificationService.get_recommended_outputs_for_pattern(
                pattern.value, ActionType.GIVEN
            )
