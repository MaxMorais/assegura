...
class ValidationRule(ABC):
    ...
    @abstractmethod
    def validate(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        """
    ...
    def validate(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        if hasattr(value, '__len__'):
            ...
            if self.min_length is not None and length < self.min_length:
                errors.append(f"Value length {length} is less than minimum {self.min_length}")
            if self.max_length is not None and length > self.max_length:
                errors.append(f"Value length {length} exceeds maximum {self.max_length}")
        else:
...
class RegexRule(ValidationRule):
    ...
    def validate(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        if not self.regex.match(str_value):
            errors.append(f"Value '{str_value}' does not match pattern '{self.pattern}'")
    ...
    def validate(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        try:
            ...
            if self.min_value is not None and num_value < self.min_value:
                errors.append(f"Value {num_value} is less than minimum {self.min_value}")
...
class ERPNextDocTypeRule(ValidationRule):
    ...
    def __init__(self, valid_doctypes: Optional[List[str]] = None):
        self.valid_doctypes = valid_doctypes or [
            "User", "Company", "Customer", "Supplier", "Item", "Sales Invoice",
            "Purchase Invoice", "Sales Order", "Purchase Order", "Quotation",
            "Delivery Note", "Purchase Receipt", "Payment Entry", "Journal Entry",
            "Lead", "Opportunity", "Project", "Task", "Timesheet", "Employee",
            "Salary Slip", "Leave Application", "Attendance", "Asset", "Stock Entry"
        ]
    ...
    def validate(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        if str_value not in self.valid_doctypes:
            errors.append(f"DocType '{str_value}' not recognized. Valid types: {', '.join(self.valid_doctypes[:10])}...")
...
class ActionParameterAdvanced:
    ...
    def validate_value(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
        """
    ...
    def _validate_dependency(self, value: Any, context: Dict[str, Any]) -> Optional[str]:
...
class ActionOutputAdvanced:
    ...
    def validate_output(self, value: Any, context: Optional[Dict[str, Any]] = None) -> List[str]:
...
class ParameterSetValidator:
    ...
    def validate_parameter_set(self, values: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for param_name, param in self.parameters.items():
            if param_name in values:
                transformed_values[param_name] = param.transform_value(values[param_name])
            else:
    ...
    def _validate_cross_parameters(self, values: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if "operation_type" in values and values["operation_type"] == "bulk":
            if not values.get("batch_size"):
                errors.setdefault("batch_size", []).append("Batch size required for bulk operations")
...
def create_doctype_parameter(name: str = "doctype", required: bool = True, **kwargs) -> ActionParameterAdvanced:
...
def create_document_name_parameter(name: str = "document_name", depends_on: str = "doctype", **kwargs) -> ActionParameterAdvanced:
...
def create_field_data_parameter(name: str = "field_data", **kwargs) -> ActionParameterAdvanced:
...
def create_selector_parameter(name: str = "selector", **kwargs) -> ActionParameterAdvanced:
...
def create_boolean_output(name: str, description: str = "", **kwargs) -> ActionOutputAdvanced:
...
def create_document_name_output(name: str = "document_name", **kwargs) -> ActionOutputAdvanced:
