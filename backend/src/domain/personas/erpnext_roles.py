...
class ERPNextRole(Enum):
    ...
    def get_module_roles(cls, module: str) -> list[str]:
        ...
        module_roles = {
            "sales": [
                ...
                cls.SALES_MASTER_MANAGER.value,
            ],
            "purchase": [
                cls.PURCHASE_MANAGER.value,
                cls.PURCHASE_USER.value,
                cls.PURCHASE_MASTER_MANAGER.value,
            ],
            ...
            "healthcare": [
                cls.HEALTHCARE_ADMINISTRATOR.value,
                cls.LABORATORY_USER.value,
                cls.NURSING_USER.value,
                cls.PHYSICIAN.value,
            ],
            "education": [
                cls.EDUCATION_MANAGER.value,
                cls.INSTRUCTOR.value,
                cls.STUDENT.value,
            ],
            "nonprofit": [
                cls.NON_PROFIT_MANAGER.value,
                cls.NON_PROFIT_PORTAL_USER.value,
            ],
    ...
    def get_role_permissions(cls, role_name: str) -> set[str]:
        ...
        role_permissions = {
            ...
            cls.CUSTOMER.value: {"read:sales", "read:accounts"},
            cls.SUPPLIER.value: {"read:purchase", "read:accounts"},
        }
    ...
    def get_hierarchical_roles(cls, role_name: str) -> list[str]:
        ...
        if (
            cls.SYSTEM_MANAGER.value not in hierarchy
            and role_name != cls.ADMINISTRATOR.value
        ):
            hierarchy.extend([cls.SYSTEM_MANAGER.value, cls.ADMINISTRATOR.value])
        elif (
            role_name != cls.ADMINISTRATOR.value
            and cls.ADMINISTRATOR.value not in hierarchy
        ):
...
def get_role_recommendations(module: str, user_level: str = "user") -> list[str]:
    ...
    if user_level.lower() == "admin":
        ...
        user_roles = [
            ...
            if "User" in role or role == ERPNextRole.EMPLOYEE.value
        ]
...
def validate_role_combination(roles: list[str]) -> list[str]:
    ...
    if ERPNextRole.ADMINISTRATOR.value in roles and len(roles) > 1:
        warnings.append(
            "Administrator role includes all permissions - other roles are redundant"
        )
    ...
    if ERPNextRole.SYSTEM_MANAGER.value in roles:
        manager_roles = [
            role
            for role in roles
            if "Manager" in role and role != ERPNextRole.SYSTEM_MANAGER.value
        ]
        if manager_roles:
            warnings.append(
                f"System Manager includes permissions from: {', '.join(manager_roles)}"
            )
    ...
    for role in roles:
        if "Manager" in role:
            ...
            if corresponding_user in roles:
                warnings.append(
                    f"{role} includes all permissions from {corresponding_user}"
                )
    ...
    if any(role in roles for role in external_roles) and internal_roles:
        warnings.append(
            "External roles (Customer/Supplier) typically should not be combined with internal roles"
        )
