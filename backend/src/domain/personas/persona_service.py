...
class PersonaService:
    ...
    def validate_persona_creation(persona: Persona) -> None:
        ...
        if len(persona.name.strip()) < 3:
            errors.append(PersonaValidationError(
    ...
    def validate_persona_update(
        ...
        if not allow_role_reduction:
            ...
            if not current_roles.issubset(updated_roles.union(current_roles)):
                ...
                if removed_roles:
                    errors.append(PersonaBusinessRuleError(
        ...
        if errors:
            if len(errors) == 1 and isinstance(errors[0], PersonaBusinessRuleError):
                ...
                validation_errors = [e for e in errors if isinstance(e, PersonaValidationError)]
    ...
    def generate_persona_suggestions(
        ...
        suggestion = {
            ...
            'permissions': PersonaService._generate_default_permissions(module, user_level),
            'module': module,
        ...
        if user_level.lower() != "admin" and ERPNextRole.EMPLOYEE.value not in recommended_roles:
            ...
            employee_suggestion['description'] = description + " Also has employee access for HR functions."
    ...
    def find_similar_personas(
        ...
        for candidate in candidate_personas:
            ...
            perm_intersection = len(persona_permissions.intersection(candidate_permissions))
            ...
            if overall_similarity >= similarity_threshold:
                similar.append({
                    ...
                    'shared_roles': list(persona_roles.intersection(candidate_roles)),
                    'shared_permissions': list(persona_permissions.intersection(candidate_permissions))
                })
    ...
    def _validate_admin_persona_rules(persona: Persona, errors: List) -> None:
        ...
        admin_roles = {ERPNextRole.ADMINISTRATOR.value, ERPNextRole.SYSTEM_MANAGER.value}
        persona_roles = set(persona.erpnext_roles)
    ...
    def _generate_default_permissions(module: str, user_level: str) -> str:
        ...
        if user_level.lower() in ["manager", "admin"]:
            permissions.extend([
                f"create:{module}_documents",
                f"delete:{module}_documents"
            ])
