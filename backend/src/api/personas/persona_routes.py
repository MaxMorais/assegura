...
from ..middleware.error_handler import handle_domain_error
...
def get_persona_service() -> PersonaService:
    ...
    from ...infrastructure.database.repositories.persona_repository import PersonaRepository
...
async def list_personas(
    limit: int = Query(default=50, ge=1, le=100, description="Maximum number of personas to return"),
    offset: int = Query(default=0, ge=0, description="Number of personas to skip"),
    search: Optional[str] = Query(default=None, description="Search in persona names and descriptions"),
    is_active: Optional[bool] = Query(default=None, description="Filter by active status"),
    erpnext_role: Optional[str] = Query(default=None, description="Filter by ERPNext role"),
    consultant_id: UUID = Depends(get_current_consultant_id),
    persona_service: PersonaService = Depends(get_persona_service)
) -> PersonaListResponse:
    ...
    """
    try:
        logger.info(f"Listing personas for consultant {consultant_id} (limit={limit}, offset={offset})")
        ...
        logger.info(f"Retrieved {len(result.personas)} personas (total: {result.total})")
        return result
...
async def get_persona_suggestions(
    ...
    """
    try:
        logger.info(f"Generating persona suggestions for context: {request.context[:50]}...")
...
@router.post("/{persona_id}/deactivate", 
             response_model=PersonaResponse,
...
async def find_similar_personas(
    ...
    """
    try:
        logger.info(f"Finding similar personas to {persona_id} (threshold={similarity_threshold})")
...
@router.get("/health",
            ...
            description="Check if persona service is healthy")
