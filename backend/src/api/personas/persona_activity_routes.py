...
async def list_persona_activities(
    ...
    try:
        ...
        activities, total = await activity_service.list_persona_activities(
            persona_id=persona_id, skip=skip, limit=limit, filters=filters
        )
...
async def get_persona_activity_link(
    persona_id: uuid.UUID, activity_id: uuid.UUID, services=Depends(get_services)
) -> ActivityPersonaLinkResponse:
...
async def bulk_link_persona_to_activities(
    ...
    try:
        ...
        return {
            ...
            "created_links": [
                ActivityPersonaLinkResponse.from_model(link) for link in results
            ],
...
async def bulk_remove_persona_activity_links(
    ...
    try:
        deleted_count = await activity_service.bulk_delete_persona_activity_links(
            persona_id=persona_id, activity_ids=bulk_data.activity_ids
        )
...
async def get_persona_activity_statistics(
    persona_id: uuid.UUID, services=Depends(get_services)
) -> dict[str, Any]:
    ...
    try:
        ...
        return {"persona_id": str(persona_id), "statistics": stats}
