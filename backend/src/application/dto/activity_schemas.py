...
class ActivityCreateRequestDTO(BaseModel):
    ...
    action_type: ActivityActionTypeDTO = Field(..., description="Type of action to perform")
    target_doctype: str = Field(..., min_length=2, description="Target ERPNext DocType")
    required_fields: List[str] = Field(default_factory=list, description="Required fields for execution")
    validation_rules: Dict[str, Any] = Field(default_factory=dict, description="Validation rules for fields")
    success_criteria: List[str] = Field(default_factory=list, description="Success criteria for completion")
    ...
    prerequisites: List[str] = Field(default_factory=list, description="Activity prerequisites")
    postconditions: List[str] = Field(default_factory=list, description="Activity postconditions")
    test_data_requirements: Dict[str, Any] = Field(default_factory=dict, description="Test data requirements")
...
class ActivityUpdateRequestDTO(BaseModel):
    ...
    name: Optional[str] = Field(None, min_length=3, max_length=100, description="Activity name")
    description: Optional[str] = Field(None, max_length=500, description="Activity description")
    complexity_score: Optional[int] = Field(None, ge=1, le=5, description="Complexity score (1-5)")
    estimated_duration: Optional[int] = Field(None, ge=1, description="Estimated duration in seconds")
    prerequisites: Optional[List[str]] = Field(None, description="Activity prerequisites")
    postconditions: Optional[List[str]] = Field(None, description="Activity postconditions")
    test_data_requirements: Optional[Dict[str, Any]] = Field(None, description="Test data requirements")
...
class ActivityFilterDTO(BaseModel):
    ...
    action_type: Optional[ActivityActionTypeDTO] = Field(None, description="Filter by action type")
    target_doctype: Optional[str] = Field(None, description="Filter by target DocType")
    complexity_score: Optional[int] = Field(None, ge=1, le=5, description="Filter by complexity score")
...
class ActivityResponseDTO(BaseModel):
    ...
    validation_rules: Dict[str, Any] = Field(..., description="Validation rules for fields")
    success_criteria: List[str] = Field(..., description="Success criteria for completion")
    ...
    test_data_requirements: Dict[str, Any] = Field(..., description="Test data requirements")
...
class ActivityStatisticsResponseDTO(BaseModel):
    ...
    by_module: Dict[str, int] = Field(..., description="Activities count by ERPNext module")
    by_action_type: Dict[str, int] = Field(..., description="Activities count by action type")
    complexity_distribution: Dict[int, int] = Field(..., description="Activities count by complexity score")
    ...
    avg_complexity: float = Field(..., ge=1, le=5, description="Average complexity score")
...
class ActivityPersonaLinkCreateRequestDTO(BaseModel):
    ...
    priority: ActivityPriorityDTO = Field(ActivityPriorityDTO.MEDIUM, description="Link priority")
    notes: str = Field("", max_length=500, description="Notes about the relationship")
    is_primary: bool = Field(False, description="Whether this is a primary activity for the persona")
...
class ActivityPersonaLinkUpdateRequestDTO(BaseModel):
    ...
    notes: Optional[str] = Field(None, max_length=500, description="Notes about the relationship")
    is_primary: Optional[bool] = Field(None, description="Whether this is a primary activity for the persona")
    execution_order: Optional[int] = Field(None, ge=0, description="Execution order in workflow")
...
class ActivityPersonaLinkResponseDTO(BaseModel):
    ...
    is_primary: bool = Field(..., description="Whether this is a primary activity for the persona")
...
class ActivityPersonaResponseDTO(BaseModel):
    ...
    persona: Dict[str, Any] = Field(..., description="Persona data")  # PersonaResponseDTO would be imported
    link: ActivityPersonaLinkResponseDTO = Field(..., description="Link metadata")
...
class ActivityCompatibilityResponseDTO(BaseModel):
    ...
    reasons: List[str] = Field(..., description="Reasons for compatibility/incompatibility")
...
class ActivitySuggestionResponseDTO(BaseModel):
    ...
    complexity_score: int = Field(..., ge=1, le=5, description="Suggested complexity score")
    estimated_duration: int = Field(..., ge=1, description="Suggested duration in seconds")
...
class ActivityBulkOperationRequestDTO(BaseModel):
    ...
    activity_ids: List[str] = Field(..., min_items=1, max_items=50, description="List of activity IDs")
...
class ActivityBulkLinkRequestDTO(BaseModel):
    ...
    activity_ids: List[str] = Field(..., min_items=1, max_items=20, description="List of activity IDs to link")
    priority: ActivityPriorityDTO = Field(ActivityPriorityDTO.MEDIUM, description="Link priority for all activities")
