...
from src.application.dto.journey_schemas import (
    JourneyCreateSchema, JourneyUpdateSchema, JourneyResponseSchema,
    JourneyListItemSchema, JourneyFilterSchema, JourneySortSchema,
    JourneyValidationSchema, JourneyExecutionPlanSchema, ActionsSummarySchema,
    JourneyAssociationUpdateSchema, JourneyStepCreateSchema, JourneyStepUpdateSchema,
    ActionStepSchema, JourneyBulkOperationSchema, JourneyBulkResultSchema,
    JourneyStatsSchema, JourneyFromTemplateSchema, JourneyTemplateSchema,
    StepExecutionResultSchema, JourneyExecutionResultSchema
)
from src.application.dto.base_schemas import PaginatedResponse
from src.domain.journeys.enhanced_journey import (
    EnhancedJourney, ActionStepEnhanced, JourneyExecutionStatus, JourneyComplexityLevel
)
from src.domain.journeys.journey_validator import JourneyValidator
from src.domain.journeys.journey_action_service import JourneyActionService, StepExecutionResult
...
class JourneyService:
    ...
    async def create_journey(self, journey_data: JourneyCreateSchema) -> JourneyResponseSchema:
        """
        ...
        try:
            ...
            if not await self.persona_service.exists(journey_data.persona_id):
                raise JourneyValidationServiceError(f"Persona {journey_data.persona_id} does not exist")
            
            if not await self.activity_service.exists(journey_data.activity_id):
                raise JourneyValidationServiceError(f"Activity {journey_data.activity_id} does not exist")
            ...
            if validation_errors:
                raise JourneyValidationServiceError(f"Journey validation failed: {validation_errors}")
    ...
    async def get_journey(self, journey_id: UUID, include_details: bool = True) -> JourneyResponseSchema:
        """
    ...
    async def list_journeys(
        ...
        try:
            ...
            items = [await self._convert_to_list_item_schema(journey) for journey in journeys]
    ...
    async def update_journey(
        ...
        try:
            ...
            if update_data.estimated_duration_minutes is not None:
                journey.update_details(estimated_duration_minutes=update_data.estimated_duration_minutes)
            if update_data.complexity_level is not None:
                journey.update_details(complexity_level=update_data.complexity_level.value)
        ...
        try:
            ...
            if association_data.persona_id:
                if not await self.persona_service.exists(association_data.persona_id):
                    raise JourneyValidationServiceError(f"Persona {association_data.persona_id} does not exist")
            
            if association_data.activity_id:
                if not await self.activity_service.exists(association_data.activity_id):
                    raise JourneyValidationServiceError(f"Activity {association_data.activity_id} does not exist")
            ...
            raise JourneyServiceError(f"Failed to update journey associations: {str(e)}")
    ...
    async def delete_journey(self, journey_id: UUID) -> bool:
        ...
        try:
            ...
            if journey.execution_status in [JourneyExecutionStatus.RUNNING, JourneyExecutionStatus.SUSPENDED]:
    ...
    async def update_journey_step(
        ...
        try:
            ...
            return await self._convert_step_to_schema(journey.enhanced_steps[step_number - 1])
    ...
    async def validate_journey(self, journey_id: UUID) -> JourneyValidationSchema:
        ...
        try:
            ...
            # Run validation
            validation_results = await self.journey_action_service.validate_journey_with_actions(journey)
    ...
    async def generate_execution_plan(self, journey_id: UUID) -> JourneyExecutionPlanSchema:
        ...
        try:
            ...
            plan_data = await self.journey_action_service.get_journey_execution_plan(journey)
    ...
    async def prepare_journey_for_execution(self, journey_id: UUID) -> bool:
        ...
        try:
            ...
            raise JourneyServiceError(f"Failed to prepare journey for execution: {str(e)}")
    
    async def bulk_operation(self, operation_data: JourneyBulkOperationSchema) -> JourneyBulkResultSchema:
            ...
            for journey_id in operation_data.journey_ids:
                try:
                    if operation_data.operation == "activate":
                        await self.update_journey(journey_id, JourneyUpdateSchema(is_active=True))
                        successful_ids.append(journey_id)
                    elif operation_data.operation == "deactivate":
                        await self.update_journey(journey_id, JourneyUpdateSchema(is_active=False))
                        successful_ids.append(journey_id)
                        ...
                        errors[str(journey_id)] = f"Unsupported operation: {operation_data.operation}"
    ...
    async def create_journey_template(self, template_data: JourneyTemplateSchema) -> str:
    ...
    async def create_journey_from_template(
        ...
        try:
            if template_data.template_name not in self._journey_templates:
                raise JourneyServiceError(f"Template '{template_data.template_name}' not found")
            ...
            raise JourneyServiceError(f"Failed to create journey from template: {str(e)}")
    
    # Private helper methods
    
    async def _add_step_to_journey(self, journey: EnhancedJourney, step_data: JourneyStepCreateSchema) -> None:
    ...
    async def _validate_journey_comprehensive(self, journey: EnhancedJourney) -> List[str]:
    ...
    async def _convert_to_response_schema(
        ...
        # Convert steps
        steps = [await self._convert_step_to_schema(step) for step in journey.enhanced_steps]
        ...
        if include_details:
            ...
            if journey.execution_plan:
                ...
                response_data["execution_plan"] = JourneyExecutionPlanSchema(**plan_data)
            
            # Add actions summary
            actions_summary = journey.get_actions_summary()
            if actions_summary:
                response_data["actions_summary"] = ActionsSummarySchema(**actions_summary)
    ...
    async def _convert_to_list_item_schema(self, journey: EnhancedJourney) -> JourneyListItemSchema:
    ...
    async def _convert_step_to_schema(self, step: ActionStepEnhanced) -> ActionStepSchema:
