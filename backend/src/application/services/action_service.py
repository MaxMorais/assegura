...
from src.application.dto.action_schemas import (
    ActionCreateSchema, ActionUpdateSchema, ActionResponseSchema,
    ActionListItemSchema, ActionFilterSchema, ActionSortSchema,
    ActionValidationResultSchema, ActionParameterSchema, ActionOutputSchema,
    ActionBulkOperationSchema, ActionBulkResultSchema, ActionSuggestionSchema,
    ActionComplexityScoreSchema, ActionUsageStatsSchema, ActionPatternSchema,
    ActionFromTemplateSchema, ActionTemplateSchema
)
from src.application.dto.base_schemas import PaginatedResponse
from src.domain.actions.action_library import (
    ActionLibrary, ActionParameter, ActionOutput, ActionType, ImplementationType
)
from src.domain.actions.action_types import ActionClassification
...
class ActionLibraryService:
    ...
    def __init__(
        self,
        action_repository: ActionRepositoryInterface
    ):
        self.action_repository = action_repository
        self.parameter_validator = ParameterValidator()
        self._action_templates: Dict[str, ActionTemplateSchema] = {}
    
    async def create_action(self, action_data: ActionCreateSchema) -> ActionResponseSchema:
        """
        ...
        try:
            ...
            action = ActionLibrary.create_action(
                ...
                implementation_type=ImplementationType(action_data.implementation_type.value),
            ...
            if validation_errors:
                raise ActionValidationServiceError(f"Action validation failed: {validation_errors}")
    ...
    async def get_actions_by_module(self, erpnext_module: str) -> List[ActionResponseSchema]:
        """Get actions for specific ERPNext module."""
    ...
    async def get_action_complexity_score(self, action_id: UUID) -> ActionComplexityScoreSchema:
    ...
    async def create_action_from_template(
        ...
        if not template:
            raise ActionLibraryServiceError(f"Template '{template_request.template_name}' not found")
    ...
    async def _convert_to_response_schema(self, action: ActionLibrary) -> ActionResponseSchema:
        ...
        return ActionResponseSchema(
            ...
            parameters=[self._convert_parameter_to_schema(p) for p in action.parameters],
    ...
    async def _convert_to_list_item_schema(self, action: ActionLibrary) -> ActionListItemSchema:
    ...
    def _convert_parameter_to_schema(self, parameter: ActionParameter) -> ActionParameterSchema:
