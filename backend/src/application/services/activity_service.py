...
from ...domain.activities.activity_service import ActivityService as ActivityDomainService
...
class ActivityApplicationService:
    ...
    def __init__(self, activity_repository, persona_repository, activity_persona_link_repository):
    ...
    async def create_activity(self, request: ActivityCreateRequestDTO) -> ActivityResponseDTO:
    ...
    async def update_activity(self, activity_id: str, request: ActivityUpdateRequestDTO) -> ActivityResponseDTO:
        ...
        # Validate with domain service
        original_activity = await self.activity_repository.get_by_id(uuid.UUID(activity_id))
    ...
    async def delete_activity(self, activity_id: str) -> None:
        ...
        await self.activity_persona_link_repository.delete_by_activity_id(uuid.UUID(activity_id))
    ...
    async def link_activity_to_persona(
        ...
        if not persona:
            raise ActivityNotFoundError(persona_id)  # Using generic error for simplicity
    ...
    async def unlink_activity_from_persona(self, persona_id: str, activity_id: str) -> None:
    ...
    async def get_persona_activities(
        ...
        for link in links:
            ...
            if activity:
                persona_activities.append(PersonaActivityResponseDTO(
    ...
    async def check_activity_compatibility(
        ...
        compatibility = self.domain_service.calculate_activity_compatibility(activity1, activity2)
    ...
    async def get_similar_activities(
        ...
        target_activity = await self.activity_repository.get_by_id(uuid.UUID(activity_id))
    ...
    async def suggest_activities(
        ...
        # Convert to DTOs
        return [
            ActivitySuggestionResponseDTO(**suggestion)
    ...
    async def bulk_link_activities(
        self, 
        persona_id: str, 
        request: ActivityBulkLinkRequestDTO
    ) -> ActivityBulkLinkResponseDTO:
        ...
        for activity_id in request.activity_ids:
            try:
                ...
                await self.link_activity_to_persona(persona_id, activity_id, link_request)
