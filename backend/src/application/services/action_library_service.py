...
from src.application.dto.action_library_schemas import (
    ActionLibraryCreateSchema, ActionLibraryUpdateSchema, ActionLibraryResponseSchema,
    ActionLibraryListItemSchema, ActionLibraryFilterSchema, ActionLibrarySortSchema,
    ActionLibraryValidationSchema, ActionLibraryBulkOperationSchema, ActionLibraryBulkResultSchema,
    ActionLibraryStatsSchema, ActionLibraryImportSchema, ActionLibraryExportSchema,
    ActionLibrarySearchSchema, ActionLibrarySearchResultSchema, ActionLibraryCategoryStatsSchema,
    ActionLibraryParameterValidationSchema, ActionLibraryExecutionMetricsSchema,
    ActionLibraryUsageStatsSchema, ActionLibraryDuplicateCheckSchema, ActionLibraryTemplateSchema,
    ActionLibraryVersionSchema, ActionLibraryRelationshipSchema
)
...
class ActionLibraryService:
    ...
    async def create_action(self, action_data: ActionLibraryCreateSchema) -> ActionLibraryResponseSchema:
        """
    ...
    async def get_action(self, action_id: UUID, include_details: bool = True) -> ActionLibraryResponseSchema:
        """
    ...
    async def search_actions(
        ...
        try:
            ...
            for action in actions:
                relevance_score = self._calculate_search_relevance(action, search_criteria)
    ...
    async def update_action(
        ...
        try:
            ...
            if any(field in update_dict for field in ['implementation', 'parameters_schema', 'expected_outputs_schema']):
    ...
    async def get_actions_by_category(self, category: ActionCategory) -> List[ActionLibraryListItemSchema]:
        """Get actions by category."""
        try:
            actions = await self.action_repository.get_actions_by_category(category)
            return [await self._convert_to_list_item_schema(action) for action in actions]
            
        except Exception as e:
            raise ActionLibraryServiceError(f"Failed to get actions by category: {str(e)}")
    
    async def get_actions_by_bdd_type(self, bdd_type: BDDStepType) -> List[ActionLibraryListItemSchema]:
        """Get actions by BDD step type."""
        try:
            ...
            raise ActionLibraryServiceError(f"Failed to get actions by BDD type: {str(e)}")
            ...
            for action_id in operation_data.action_ids:
                try:
                    if operation_data.operation == "activate":
                        await self.update_action(action_id, ActionLibraryUpdateSchema(is_active=True))
                        successful_ids.append(action_id)
                    elif operation_data.operation == "deactivate":
                        await self.update_action(action_id, ActionLibraryUpdateSchema(is_active=False))
                        successful_ids.append(action_id)
                    elif operation_data.operation == "delete":
                        await self.delete_action(action_id, force=operation_data.parameters.get("force", False))
                        successful_ids.append(action_id)
                        ...
                        errors[str(action_id)] = f"Unsupported operation: {operation_data.operation}"
    ...
    async def get_action_statistics(self) -> ActionLibraryStatsSchema:
        ...
        try:
            ...
            for category in ActionCategory:
                category_actions = await self.action_repository.get_actions_by_category(category)
                category_stats.append(ActionLibraryCategoryStatsSchema(
                    category=category.value,
                    action_count=len(category_actions),
                    active_count=len([a for a in category_actions if a.is_active]),
                    average_usage=sum(a.usage_count for a in category_actions) / len(category_actions) if category_actions else 0
                ))
            ...
            raise ActionLibraryServiceError(f"Failed to get action statistics: {str(e)}")
    ...
    async def export_actions(
        ...
        try:
            ...
            filtered_actions = []
            for action in actions:
                if export_criteria.categories and action.category not in export_criteria.categories:
                    continue
                if export_criteria.bdd_step_types and action.bdd_step_type not in export_criteria.bdd_step_types:
                    continue
                if export_criteria.tags and not any(tag in action.tags for tag in export_criteria.tags):
                    continue
            ...
            for action_data in import_data.actions:
                try:
                    ...
                    if "name" in action_data:
                        existing_action = await self.action_repository.get_by_name(action_data["name"])
                    
                    if existing_action and import_data.duplicate_strategy == "skip":
                        continue
                    elif existing_action and import_data.duplicate_strategy == "replace":
                        # Update existing action
                        update_schema = ActionLibraryUpdateSchema(**action_data)
                        updated_action = await self.update_action(existing_action.action_id, update_schema)
    ...
    async def create_action_template(self, template_data: ActionLibraryTemplateSchema) -> str:
        ...
        try:
            ...
            raise ActionLibraryServiceError(f"Failed to create action template: {str(e)}")
    
    async def get_action_relationships(self, action_id: UUID) -> ActionLibraryRelationshipSchema:
        ...
        try:
            ...
            relationships = await self.action_relationship_service.get_action_relationships(action)
            ...
            raise ActionLibraryServiceError(f"Failed to get action relationships: {str(e)}")
    ...
    async def _check_for_duplicates(
        ...
        return ActionLibraryDuplicateCheckSchema(
            ...
            similarity_scores={
                str(action.action_id): self._calculate_similarity_score(temp_action, action)
                for action in duplicates
            }
        )
    
    def _calculate_similarity_score(self, action1: EnhancedActionLibrary, action2: EnhancedActionLibrary) -> float:
        ...
        if action1.name.lower() == action2.name.lower():
            score += 0.4
        elif action1.name.lower() in action2.name.lower() or action2.name.lower() in action1.name.lower():
            score += 0.2
        ...
        if impl1_keywords and impl2_keywords:
            ...
            score += 0.1 * (len(common_keywords) / max(len(impl1_keywords), len(impl2_keywords)))
    ...
    def _calculate_search_relevance(
        ...
        # BDD step type match
        if search_criteria.bdd_step_types and action.bdd_step_type in search_criteria.bdd_step_types:
            score += 0.5
    ...
    def _validate_parameters(self, action: EnhancedActionLibrary) -> ActionLibraryParameterValidationSchema:
    ...
    def _validate_implementation(self, action: EnhancedActionLibrary) -> Dict[str, Any]:
        ...
        try:
            ...
            if not action.implementation:
                ...
                validation_results["implementation_errors"].append("Implementation is required")
    ...
    async def _convert_to_response_schema(
        ...
        if include_details:
            ...
            if hasattr(action, 'execution_metrics') and action.execution_metrics:
                response_data["execution_metrics"] = ActionLibraryExecutionMetricsSchema(
    ...
    async def _convert_to_list_item_schema(self, action: EnhancedActionLibrary) -> ActionLibraryListItemSchema:
