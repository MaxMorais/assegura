...
from src.infrastructure.database.models.base import BaseModel, TimestampMixin, AuditMixin
...
class ActionLibraryModel(BaseModel, TimestampMixin, AuditMixin):
    ...
    __table_args__ = (
        ...
        Index('ix_action_quality_metrics', 'success_rate', 'average_execution_time_seconds'),
        Index('ix_action_execution_config', 'default_timeout_seconds', 'default_retry_count'),
        ...
        Index('ix_action_parameters_schema_gin', 'parameters_schema', postgresql_using='gin'),
        Index('ix_action_expected_outputs_gin', 'expected_outputs_schema', postgresql_using='gin'),
        ...
        # Text search indexes
        Index('ix_action_name_text', 'name', postgresql_using='gin', postgresql_ops={'name': 'gin_trgm_ops'}),
        Index('ix_action_description_text', 'description', postgresql_using='gin', postgresql_ops={'description': 'gin_trgm_ops'}),
    )
...
class ActionExecutionMetricsModel(BaseModel, TimestampMixin):
    ...
    stability_rating = Column(String(20), nullable=True, index=True)  # stable, unstable, unreliable
    ...
    __table_args__ = (
        ...
        Index('ix_metrics_execution_stats', 'total_executions', 'successful_executions', 'failed_executions'),
        ...
        # JSON indexes
        Index('ix_metrics_failure_reasons_gin', 'common_failure_reasons', postgresql_using='gin'),
        Index('ix_metrics_failure_patterns_gin', 'failure_patterns', postgresql_using='gin'),
        Index('ix_metrics_performance_trends_gin', 'performance_trends', postgresql_using='gin'),
        Index('ix_metrics_resource_usage_gin', 'resource_usage_stats', postgresql_using='gin'),
    )
...
class ActionVersionModel(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('action_id', 'version_number', name='uq_action_version_number'),
        ...
        # JSON indexes
        Index('ix_version_change_details_gin', 'change_details', postgresql_using='gin'),
        Index('ix_version_breaking_changes_gin', 'breaking_changes', postgresql_using='gin'),
    )
...
class ActionRelationshipModel(BaseModel, TimestampMixin):
    ...
    conditions = Column(JSONB, nullable=True, default=dict)  # When this relationship applies
    metadata = Column(JSONB, nullable=True, default=dict)
    ...
    context_tags = Column(JSONB, nullable=True, default=list)  # Contexts where relationship applies
    ...
    target_action = relationship(
        "ActionLibraryModel", 
        foreign_keys=[target_action_id]
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            'source_action_id', 'target_action_id', 'relationship_type',
        ...
        Index('ix_relationship_type_strength', 'relationship_type', 'relationship_strength'),
        ...
        Index('ix_relationship_context_tags_gin', 'context_tags', postgresql_using='gin'),
    )
...
class ActionUsageTrackingModel(BaseModel, TimestampMixin):
    ...
    # Usage context
    usage_context = Column(String(100), nullable=False, index=True)  # journey, test, manual, api
    usage_environment = Column(String(50), nullable=True, index=True)  # dev, staging, prod
    ...
    execution_result = Column(String(50), nullable=True, index=True)  # success, failure, error
    ...
    __table_args__ = (
        ...
        Index('ix_usage_result_duration', 'execution_result', 'execution_duration_seconds'),
    ...
    def __repr__(self):
        return f"<ActionUsageTracking(action_id={self.action_id}, used_at={self.used_at})>"