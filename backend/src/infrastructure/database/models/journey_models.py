...
from sqlalchemy import (
    ...
    Index, CheckConstraint, UniqueConstraint, ARRAY
)
...
class JourneyModel(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('name', 'persona_id', 'activity_id', name='uq_journey_name_persona_activity'),
        ...
        Index('ix_journey_complexity_duration', 'complexity_level', 'estimated_duration_minutes'),
        ...
        Index('ix_journey_expected_outcomes_gin', 'expected_outcomes', postgresql_using='gin'),
    )
...
class JourneyStepModel(BaseModel, TimestampMixin):
    ...
    depends_on_steps = Column(JSONB, nullable=True, default=list)  # Array of step numbers
    ...
    __table_args__ = (
        ...
        Index('ix_journey_step_expected_outputs_gin', 'expected_outputs', postgresql_using='gin'),
        Index('ix_journey_step_depends_on_gin', 'depends_on_steps', postgresql_using='gin'),
        Index('ix_journey_step_execution_result_gin', 'execution_result', postgresql_using='gin'),
    )
...
class JourneyExecutionPlanModel(BaseModel, TimestampMixin):
    ...
    plan_generated_at = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)
    ...
    __table_args__ = (
        ...
        Index('ix_execution_plan_duration_complexity', 'estimated_duration_seconds', 'complexity_score'),
        Index('ix_execution_plan_parallelization', 'can_execute_parallel'),
        
        # JSON indexes
        Index('ix_execution_plan_parallel_steps_gin', 'parallel_executable_steps', postgresql_using='gin'),
        Index('ix_execution_plan_critical_path_gin', 'critical_path_steps', postgresql_using='gin'),
        Index('ix_execution_plan_rollback_points_gin', 'rollback_points', postgresql_using='gin'),
        Index('ix_execution_plan_resource_requirements_gin', 'resource_requirements', postgresql_using='gin'),
    )
...
class JourneyExecutionModel(BaseModel, TimestampMixin):
    ...
    execution_batch_id = Column(PG_UUID(as_uuid=True), nullable=True, index=True)  # For batch executions
    ...
    final_result = Column(String(50), nullable=True, index=True)  # success, failure, partial_success, cancelled
    ...
    execution_context = Column(JSONB, nullable=True, default=dict)  # Environment, user, etc.
    ...
    execution_environment = Column(String(100), nullable=True, index=True)  # dev, staging, prod
    ...
    __table_args__ = (
        ...
        UniqueConstraint('journey_id', 'execution_number', name='uq_journey_execution_number'),
        ...
        # JSON indexes
        Index('ix_journey_execution_context_gin', 'execution_context', postgresql_using='gin'),
        Index('ix_journey_execution_results_gin', 'execution_results', postgresql_using='gin'),
        Index('ix_journey_execution_performance_gin', 'performance_metrics', postgresql_using='gin'),
    )
...
class JourneyStepExecutionModel(BaseModel, TimestampMixin):
    ...
    step_number = Column(Integer, nullable=False, index=True)
    execution_order = Column(Integer, nullable=False, index=True)  # Actual execution order (may differ from step_number for parallel execution)
    ...
    execution_result = Column(String(50), nullable=True, index=True)  # passed, failed, skipped, error
    
    # Step execution data
    input_parameters = Column(JSONB, nullable=True, default=dict)
    actual_outputs = Column(JSONB, nullable=True, default=dict)
    step_artifacts = Column(JSONB, nullable=True, default=dict)  # Screenshots, logs, etc.
    ...
    # Relationships
    journey_execution = relationship("JourneyExecutionModel", back_populates="step_executions")
    journey_step = relationship("JourneyStepModel")
    
    # Table constraints
    __table_args__ = (
        # Unique constraint on step execution within journey execution
        UniqueConstraint('journey_execution_id', 'step_number', name='uq_journey_step_execution'),
        ...
        Index('ix_step_execution_status_result', 'execution_status', 'execution_result'),
        Index('ix_step_execution_performance', 'duration_seconds'),
        Index('ix_step_execution_retry', 'retry_count'),
        
        # JSON indexes
        Index('ix_step_execution_input_parameters_gin', 'input_parameters', postgresql_using='gin'),
        Index('ix_step_execution_actual_outputs_gin', 'actual_outputs', postgresql_using='gin'),
        Index('ix_step_execution_artifacts_gin', 'step_artifacts', postgresql_using='gin'),
        Index('ix_step_execution_error_details_gin', 'error_details', postgresql_using='gin'),
        Index('ix_step_execution_performance_gin', 'performance_data', postgresql_using='gin'),
    )
