...
class DatabaseConfig(BaseSettings):
    ...
    enable_multi_tenant: bool = Field(default=True, description="Enable multi-tenant support")
    
    # SSL Configuration
    db_ssl_mode: str = Field(default="prefer", description="SSL mode (disable/allow/prefer/require)")
    ...
    def get_database_url(self, include_password: bool = True) -> str:
        ...
        ssl_part = f"?sslmode={self.db_ssl_mode}" if self.db_ssl_mode != "disable" else ""
...
class TestDatabaseConfig(DatabaseConfig):
    ...
    db_name: str = Field(default="erpnext_test_automation_test", description="Test database name")
