...
class SQLAlchemyPersonaRepository(PersonaRepository):
        ...
        try:
            ...
            if persona.id:
                existing_model = (
                    ...
                    .filter(PersonaModel.id == persona.id)
                    .first()
                )
                ...
                if existing_model:
                    ...
                    if existing_model.version != persona.version:
                        raise PersonaConcurrencyError(
                            str(persona.id), persona.version, existing_model.version
                        )
    ...
    async def find_by_name(self, name: str) -> Optional[Persona]:
        ...
        model = (
            self.session.query(PersonaModel).filter(PersonaModel.name == name).first()
        )
    ...
    async def find_active(self) -> list[Persona]:
        ...
        models = (
            self.session.query(PersonaModel)
            .filter(PersonaModel.is_active == True)
            .order_by(PersonaModel.name)
            .all()
        )
    ...
    async def find_with_pagination(
        ...
        if filters:
            ...
            if "created_after" in filters:
                query = query.filter(
                    PersonaModel.created_at >= filters["created_after"]
                )
    ...
    async def exists_by_name(
        ...
        query = self.session.query(PersonaModel).filter(PersonaModel.name == name)
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        for model in complex_personas:
            ...
            perm_count = len(
                [p for p in (model.permissions or "").split(",") if p.strip()]
            )
    ...
    async def find_created_after(self, date: datetime) -> list[Persona]:
        ...
        models = (
            self.session.query(PersonaModel)
            .filter(PersonaModel.created_at >= date)
            .order_by(PersonaModel.created_at.desc())
            .all()
        )
    ...
    async def find_updated_after(self, date: datetime) -> list[Persona]:
        ...
        models = (
            self.session.query(PersonaModel)
            .filter(PersonaModel.updated_at >= date)
            .order_by(PersonaModel.updated_at.desc())
            .all()
        )
    ...
    async def get_creation_trend(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        query = (
            ...
            .filter(PersonaModel.created_at >= start_date)
            .group_by(func.date(PersonaModel.created_at))
            .order_by(func.date(PersonaModel.created_at))
        )
    ...
    async def find_similar_by_roles(
        ...
        models = (
            self.session.query(PersonaModel).filter(PersonaModel.id != persona.id).all()
        )
        ...
        for model in models:
            model_roles = set(
                [r.strip() for r in model.erpnext_roles.split(",") if r.strip()]
            )
    ...
    async def bulk_update_status(self, persona_ids: list[UUID], is_active: bool) -> int:
        ...
        result = (
            self.session.query(PersonaModel)
            .filter(PersonaModel.id.in_(persona_ids))
            .update({PersonaModel.is_active: is_active}, synchronize_session=False)
        )
    ...
    async def cleanup_inactive(self, days_inactive: int = 90) -> int:
        ...
        inactive_personas = self.session.query(PersonaModel).filter(
            and_(PersonaModel.is_active == False, PersonaModel.updated_at < cutoff_date)
        )
