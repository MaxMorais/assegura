...
from src.infrastructure.database.models.action_library_models import (
    ActionLibraryModel, ActionParameterModel, ActionOutputModel
)
...
class SQLAlchemyActionRepository(BaseRepository, ActionRepositoryInterface):
    ...
    async def create(self, action: ActionLibrary) -> ActionLibrary:
        ...
        try:
            ...
            self.db.rollback()
            raise ActionRepositoryError(f"Action creation failed due to constraint violation: {e}")
    ...
    async def update(self, action: ActionLibrary) -> ActionLibrary:
        ...
        try:
            ...
            if not action_model:
                raise ActionRepositoryError(f"Action with ID {action.action_id} not found")
            ...
            self.db.rollback()
            raise ActionRepositoryError(f"Action update failed due to constraint violation: {e}")
        except SQLAlchemyError as e:
            self.db.rollback()
    ...
    async def get_actions_by_module(self, erpnext_module: str) -> List[ActionLibrary]:
        ...
        try:
            ...
            raise ActionRepositoryError(f"Database error during module actions retrieval: {e}")
    ...
    async def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "by_module": {item.erpnext_module or "unassigned": item.count for item in by_module}
            }
            
        except SQLAlchemyError as e:
            raise ActionRepositoryError(f"Database error during statistics retrieval: {e}")
    ...
    async def bulk_update_status(
        ...
        try:
            ...
            self.db.rollback()
            raise ActionRepositoryError(f"Database error during bulk status update: {e}")
    ...
    def _apply_filters(self, query, filters: ActionFilterSchema):
        ...
        if filters.action_type:
            query = query.filter(ActionLibraryModel.action_type == filters.action_type.value)
        
        if filters.implementation_type:
            query = query.filter(
                ActionLibraryModel.implementation_type == filters.implementation_type.value
            )
        
        if filters.erpnext_module:
            query = query.filter(ActionLibraryModel.erpnext_module == filters.erpnext_module)
        ...
        if filters.description_contains:
            query = query.filter(
                ActionLibraryModel.description.ilike(f"%{filters.description_contains}%")
            )
    ...
    def _apply_sorting(self, query, sort: ActionSortSchema):
        ...
        sort_column = getattr(ActionLibraryModel, sort.sort_by, ActionLibraryModel.updated_at)
    ...
    def _update_model_from_domain(self, model: ActionLibraryModel, action: ActionLibrary) -> None:
    ...
    async def _convert_model_to_domain(self, model: ActionLibraryModel) -> ActionLibrary:
