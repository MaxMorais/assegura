...
from ...auth.models import Consultant
from ...auth.models import ConsultantRole
from ...auth.models import Tenant
from ...auth.models import TenantStatus
...
class SqlTenantRepository(BaseRepository[Tenant, TenantModel], TenantRepositoryInterface):
    ...
    async def exists_by_subdomain(self, subdomain: str) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to check tenant existence by subdomain {subdomain}: {e}")


class SqlConsultantRepository(MultiTenantRepository[Consultant, ConsultantModel], ConsultantRepositoryInterface):
    ...
    async def find_by_email(self, email: str, tenant_id: UUID) -> Optional[Consultant]:
        ...
        try:
            ...
            if tenant_id != self.tenant_id:
                raise ValueError(f"Tenant ID mismatch: expected {self.tenant_id}, got {tenant_id}")
            
            filters = {"email": email.lower()}
            results = await self.find_by_filters(filters, limit=1)
            return results[0] if results else None
        except Exception as e:
            raise RuntimeError(f"Failed to find consultant by email {email} in tenant {tenant_id}: {e}")
    ...
    async def find_by_tenant(self, tenant_id: UUID) -> List[Consultant]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find consultants for tenant {tenant_id}: {e}")
    ...
    async def find_active_by_tenant(self, tenant_id: UUID) -> List[Consultant]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find active consultants for tenant {tenant_id}: {e}")
    
    async def find_by_role(self, role: ConsultantRole, tenant_id: UUID) -> List[Consultant]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find consultants by role {role} for tenant {tenant_id}: {e}")
    ...
    async def exists_by_email(self, email: str, tenant_id: UUID) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to check consultant existence by email {email} in tenant {tenant_id}: {e}")
    ...
    async def count_by_tenant(self, tenant_id: UUID) -> int:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to count consultants for tenant {tenant_id}: {e}")
    ...
    async def count_by_role(self, role: ConsultantRole, tenant_id: UUID) -> int:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to count consultants by role {role} for tenant {tenant_id}: {e}")
...
def create_consultant_repository(session: Session, tenant_id: UUID) -> ConsultantRepositoryInterface:
