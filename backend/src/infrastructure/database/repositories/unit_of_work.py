...
from src.infrastructure.auth.repositories import ConsultantRepository, TenantRepository
from .auth_repositories import create_consultant_repository, create_tenant_repository
...
class SqlUnitOfWork(UnitOfWork):
    ...
    def get_repository(
        self, repository_class: type[BaseRepository], *args, **kwargs
    ) -> BaseRepository:
        ...
        if repo_key not in self._repositories:
            if issubclass(repository_class, MultiTenantRepository):
                if not self.tenant_id:
                    raise ValueError(
                        f"tenant_id is required for multi-tenant repository {repository_class.__name__}"
                    )
                self._repositories[repo_key] = repository_class(
                    self.session, self.tenant_id, *args, **kwargs
                )
            else:
                self._repositories[repo_key] = repository_class(
...
def create_unit_of_work(
    session: Session, tenant_id: Optional[UUID] = None
) -> Generator[SqlUnitOfWork, None, None]:
...
async def create_async_unit_of_work(
    session: AsyncSession, tenant_id: Optional[UUID] = None
...
class UnitOfWorkFactory:
    ...
    def __init__(
        ...
        async_session_factory: Optional[callable] = None,
    ) -> None:
