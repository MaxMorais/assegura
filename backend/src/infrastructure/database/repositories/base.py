...
class BaseRepository(Repository[EntityType], Generic[EntityType, ModelType], ABC):
    ...
    async def find_all(self, limit: Optional[int] = None, offset: Optional[int] = None) -> List[EntityType]:
...
class MultiTenantRepository(BaseRepository[EntityType, ModelType], Generic[EntityType, ModelType], ABC):
    ...
    async def find_by_id(self, entity_id: UUID) -> Optional[EntityType]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find entity by ID {entity_id} in tenant {self.tenant_id}: {e}")
    ...
    async def find_all(self, limit: Optional[int] = None, offset: Optional[int] = None) -> List[EntityType]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find all entities for tenant {self.tenant_id}: {e}")
    ...
    async def count(self) -> int:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to count entities for tenant {self.tenant_id}: {e}")
    ...
    async def find_by_filters(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to find entities by filters for tenant {self.tenant_id}: {e}")
    ...
    async def count_by_filters(self, filters: Dict[str, Any]) -> int:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to count entities by filters for tenant {self.tenant_id}: {e}")
