...
class ActionLibraryRepository(BaseRepository, ActionLibraryRepositoryInterface):
    ...
    async def create(self, action: EnhancedActionLibrary) -> EnhancedActionLibrary:
        ...
        try:
            ...
            version_model = ActionVersionModel(
                action_id=action_model.id,
                version_number=1,
                implementation_hash=self._calculate_implementation_hash(
                    action.implementation
                ),
                parameters_schema_hash=self._calculate_schema_hash(
            ...
            if "unique constraint" in str(e).lower():
                raise ActionLibraryRepositoryError(
                    f"Action with name '{action.name}' already exists"
                )
            raise ActionLibraryRepositoryError(
                f"Action creation failed due to constraint violation: {str(e)}"
            )
        except SQLAlchemyError as e:
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Database error during action creation: {str(e)}"
            )
        except Exception as e:
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Unexpected error during action creation: {str(e)}"
            )
    ...
    async def get_by_id(self, action_id: UUID) -> Optional[EnhancedActionLibrary]:
        ...
        try:
            ...
            raise ActionLibraryRepositoryError(
                f"Database error retrieving action {action_id}: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
    ...
    async def get_by_name(self, name: str) -> Optional[EnhancedActionLibrary]:
        ...
        try:
            ...
            raise ActionLibraryRepositoryError(
                f"Database error retrieving action '{name}': {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error retrieving action '{name}': {str(e)}"
            )
    ...
    async def list_actions(
        ...
        try:
            ...
            query = self.db_session.query(ActionLibraryModel).options(
                selectinload(ActionLibraryModel.execution_metrics),
                selectinload(ActionLibraryModel.versions),
            )
            ...
            if sort:
                ...
                query = query.order_by(
                    desc(ActionLibraryModel.usage_count), asc(ActionLibraryModel.name)
                )
            ...
            raise ActionLibraryRepositoryError(
                f"Database error listing actions: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
        ...
        try:
            ...
            if search_criteria.categories:
                category_values = [
                    ...
                    for cat in search_criteria.categories
                ]
            ...
            if search_criteria.action_types:
                type_values = [
                    at.value if hasattr(at, "value") else at
                    for at in search_criteria.action_types
                ]
            ...
            if search_criteria.bdd_step_types:
                bdd_values = [
                    bst.value if hasattr(bst, "value") else bst
                    for bst in search_criteria.bdd_step_types
                ]
            ...
            if search_criteria.erpnext_doctype:
                query = query.filter(
                    ...
                    == search_criteria.erpnext_doctype
                )

            # Active status filter
            if search_criteria.is_active is not None:
                query = query.filter(
                    ActionLibraryModel.is_active == search_criteria.is_active
                )

            # Usage count range
            if search_criteria.min_usage_count is not None:
                query = query.filter(
                    ActionLibraryModel.usage_count >= search_criteria.min_usage_count
                )

            if search_criteria.max_usage_count is not None:
                query = query.filter(
                    ActionLibraryModel.usage_count <= search_criteria.max_usage_count
                )

            # Date range filters
            if search_criteria.created_after:
                query = query.filter(
                    ActionLibraryModel.created_at >= search_criteria.created_after
                )

            if search_criteria.created_before:
                query = query.filter(
                    ActionLibraryModel.created_at <= search_criteria.created_before
                )
            ...
            if search_criteria.text_query:
                ...
                query = query.add_columns(relevance_score).order_by(
                    desc("relevance"), desc(ActionLibraryModel.usage_count)
                )
                results = query.all()
                action_models = [
            ...
            raise ActionLibraryRepositoryError(
                f"Database error searching actions: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error searching actions: {str(e)}"
            )
    ...
    async def update(self, action: EnhancedActionLibrary) -> EnhancedActionLibrary:
        ...
        try:
            ...
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Database error updating action: {str(e)}"
            )
        except Exception as e:
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Unexpected error updating action: {str(e)}"
            )
    ...
    async def delete(self, action_id: UUID) -> bool:
        ...
        try:
            ...
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Database error deleting action: {str(e)}"
            )
        except Exception as e:
            self.db_session.rollback()
            raise ActionLibraryRepositoryError(
                f"Unexpected error deleting action: {str(e)}"
            )
    ...
    async def get_actions_by_category(
        ...
        try:
            ...
            raise ActionLibraryRepositoryError(
                f"Database error getting actions by category: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error getting actions by category: {str(e)}"
            )
    ...
    async def get_actions_by_bdd_type(
        ...
        try:
            ...
            raise ActionLibraryRepositoryError(
                f"Database error getting actions by BDD type: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error getting actions by BDD type: {str(e)}"
            )
    ...
    async def get_action_stats(self) -> dict[str, Any]:
        ...
        try:
            # Basic counts
            total_actions = self.db_session.query(
                func.count(ActionLibraryModel.id)
            ).scalar()
            active_actions = (
            ...
            # Usage statistics
            usage_stats = self.db_session.query(
            ...
            most_used = (
                self.db_session.query(
                    ActionLibraryModel.name, ActionLibraryModel.usage_count
                )
            ...
            return {
                ...
                "bdd_step_type_distribution": {
                    bdd_type: count for bdd_type, count in bdd_stats
                },
                "action_type_distribution": {
                    action_type: count for action_type, count in type_stats
                },
                "usage_statistics": {
                ...
                "most_used_actions": [
                    {"name": name, "usage_count": count} for name, count in most_used
                ],
                "erpnext_doctype_distribution": {
                    doctype: count for doctype, count in doctype_stats
                },
                "quality_metrics": {
            ...
            raise ActionLibraryRepositoryError(
                f"Database error getting action stats: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error getting action stats: {str(e)}"
            )
    ...
    async def get_duplicate_candidates(
        ...
        try:
            ...
            for candidate in candidates:
                ...
                if (
                    candidate.category == action.category.value
                    and candidate.bdd_step_type == action.bdd_step_type.value
                ):
                    potential_duplicates.append(candidate)
                # Check ERPNext doctype similarity
                elif (
                    candidate.erpnext_doctype
                    and action.erpnext_doctype
                    and candidate.erpnext_doctype == action.erpnext_doctype
                ):
                    potential_duplicates.append(candidate)
            ...
            raise ActionLibraryRepositoryError(
                f"Database error finding duplicates: {str(e)}"
            )
        except Exception as e:
            raise ActionLibraryRepositoryError(
                f"Unexpected error finding duplicates: {str(e)}"
            )
    ...
    def _apply_filters(self, query, filters: ActionLibraryFilterSchema):
        ...
        if filters.action_types:
            type_values = [
                at.value if hasattr(at, "value") else at for at in filters.action_types
            ]
            query = query.filter(ActionLibraryModel.action_type.in_(type_values))
        ...
        if filters.bdd_step_types:
            bdd_values = [
                bst.value if hasattr(bst, "value") else bst
                for bst in filters.bdd_step_types
            ]
            query = query.filter(ActionLibraryModel.bdd_step_type.in_(bdd_values))
        ...
        if filters.erpnext_doctype:
            query = query.filter(
                ActionLibraryModel.erpnext_doctype == filters.erpnext_doctype
            )

        if filters.min_usage_count is not None:
            query = query.filter(
                ActionLibraryModel.usage_count >= filters.min_usage_count
            )

        if filters.max_usage_count is not None:
            query = query.filter(
                ActionLibraryModel.usage_count <= filters.max_usage_count
            )
    ...
    def _update_model_from_domain(
        self, model: ActionLibraryModel, action: EnhancedActionLibrary
    ) -> None:
    ...
    def _check_needs_versioning(
        self, model: ActionLibraryModel, action: EnhancedActionLibrary
    ) -> bool:
        """Check if changes require creating a new version."""

        # Check for significant changes
        implementation_changed = self._calculate_implementation_hash(
            action.implementation
        ) != self._calculate_implementation_hash(model.implementation)

        schema_changed = self._calculate_schema_hash(
            action.parameters_schema
    ...
    async def _convert_model_to_domain(
        ...
        if hasattr(model, "execution_metrics") and model.execution_metrics:
            action.execution_metrics = await self._convert_metrics_model_to_domain(
    ...
    async def _convert_metrics_model_to_domain(
        ...
        return ActionExecutionMetrics(
            ...
            success_rate=metrics_model.successful_executions
            / max(metrics_model.total_executions, 1),
