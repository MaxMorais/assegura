...
class Permission(ValueObject):
    ...
    def matches(self, resource: str, action: str, scope: Optional[str] = None) -> bool:
        ...
        """
        return (
            self.resource == resource.lower() and
            self.action == action.lower() and
            (self.scope is None or self.scope == (scope.lower() if scope else None))
        )
...
class Tenant(AggregateRoot):
    ...
    def __init__(
        ...
        if not self.subdomain.replace('-', '').replace('_', '').isalnum():
            raise ValueError("Subdomain must be alphanumeric with hyphens or underscores")
