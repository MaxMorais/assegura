...
from .models import Consultant, Tenant
...
class AuthenticationService:
    ...
    def __init__(
        self, token_service: TokenService, password_service: PasswordService
    ) -> None:
    ...
    def authenticate_consultant(
        self, email: str, password: str, tenant: Tenant, consultant: Consultant
        ...
        if not self.password_service.verify_password(
            password, consultant.password_hash
        ):
            return None
    ...
    def refresh_access_token(
        self, refresh_token: str, consultant: Consultant, tenant: Tenant
    ) -> Optional[str]:
        ...
        if payload.get("sub") != str(consultant.id) or payload.get("tenant_id") != str(
            tenant.id
        ):
            return None
    ...
    def change_password(
        self, consultant: Consultant, current_password: str, new_password: str
    ) -> bool:
        ...
        if not self.password_service.verify_password(
            current_password, consultant.password_hash
        ):
            return False
